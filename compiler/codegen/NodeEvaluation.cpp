    ...
    if (opcode != TR::BBStart && node->getRegister()) {
        ...
        while (_stackOfArtificiallyInflatedNodes.topIndex() > topOfNodeStackBeforeEvaluation) {
            ...
            if (artificiallyInflatedNode->getReferenceCount() == 1) {
                ...
                //
                // 1a) N is never evaluated, so the ref count never goes down to 1. (node was not commoned in another
                // subtree)
                //
        ...
        bool checkRefCount = (node->getReferenceCount() <= 1 || (reg && reg == node->getRegister()));
...
int32_t OMR::CodeGenerator::whichNodeToEvaluate(TR::Node *first, TR::Node *second)
{
...
int32_t OMR::CodeGenerator::whichChildToEvaluate(TR::Node *node)
{
    // Use tree depth as an indicator of priority.
    //
...
rcount_t OMR::CodeGenerator::incReferenceCount(TR::Node *node)
{
...
rcount_t OMR::CodeGenerator::decReferenceCount(TR::Node *node)
{
    TR::Register *reg = node->getRegister();

    if ((node->getReferenceCount() == 1) && reg && self()->getLiveRegisters(reg->getKind())) {
        ...
        self()->comp()->log()->printf("zzz decReferenceCount - node: %p, reg: %p, liveRegister: %p, regIsAlive: %d\n", node, reg, liveRegister, reg->isLive());
...
void OMR::CodeGenerator::evaluateChildrenWithMultipleRefCount(TR::Node *node)
{
