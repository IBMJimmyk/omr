...
static TR::ILOpCodes geta2xOpCode(TR::DataType testType)
   {
   if (testType.isInt64())
      return TR::a2l;
   TR_ASSERT(testType.isInt32(), "unsupported type");
   return TR::a2i;
   }
...
//          {
         ...
         TR::ILOpCodes addOpCode = getIndexType().isAddress() ?
            (getTestChildType().isInt64() ? TR::aladd : TR::aiadd) :
            (getTestChildType().isInt64() ? TR::ladd  : TR::iadd)  ;
         tempValue = TR::Node::create(addOpCode, 2,originalLimitClone, constNode);
         ...
         storeOpCode = getIndexType().isAddress() ?
            TR::astore :
            (getTestChildType().isInt64() ? TR::lstore : TR::istore);
         //         }
      ...
      if (getIndexType().isAddress())
         {
         newLimitNode = TR::Node::create(geta2xOpCode(getTestChildType()), 1, newLimitNode);
         }
      ...
      if (getIndexType().isAddress())
         {
         opCode = getTestChildType().isInt32() ?
            (isIncreasingLoop() ? TR::ifiucmplt : TR::ifiucmpgt) :
            (isIncreasingLoop() ? TR::iflucmplt : TR::iflucmpgt) ;
         }
      else
         {
         if (getTestIsUnsigned())
      ...
      if (getIndexType().isAddress())
         c2 = TR::Node::create(geta2xOpCode(getTestChildType()), 1, c2);
   ...
   if (1) /// moveEntryNode
      ...
      if (loop->getPrimaryInductionVariable() &&
         ...
         if (dupNewLimit->getDataType() == TR::Int32)
            {
      ...
      if (1)
         ...
         if (1)
            ...
            if (1)
               ...
               if (getIndexType().isAddress())
                  {
                  c1 = TR::Node::create(geta2xOpCode(getTestChildType()), 1, c1);
                  }
               ...
               if (loop->getPrimaryInductionVariable() &&
                  ...
                  if (!getTestIsUnsigned())
                     ...
                     if (ifNode->getFirstChild()->getDataType() == TR::Int32)
                        {
