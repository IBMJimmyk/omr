...
extern TR::TreeTop *createStoresForArraycopyChildren(TR::Compilation *comp, TR::TreeTop *arrayTree,
    TR::SymbolReference *&srcObjRef, TR::SymbolReference *&dstObjRef, TR::SymbolReference *&srcRef,
    TR::SymbolReference *&dstRef, TR::SymbolReference *&lenRef);
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::createRelationship(int32_t relative,
...
void OMR::ValuePropagation::freeRelationship(Relationship *rel) { _relationshipCache.add(rel); }
...
OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::createStoreRelationship(TR::Symbol *symbol,
...
bool OMR::ValuePropagation::propagateConstraint(TR::Node *node, int32_t valueNumber, Relationship *first,
    Relationship *rel, ValueConstraints *valueConstraints)
{
    ...
    if (_propagationDepth > _maxPropagationDepth) {
        ...
        logprintf(trace(), comp()->log(), "===>Reached Max Relational Propagation Depth: %d\n", _propagationDepth);
    ...
    for (cur = first; cur && worthPropagatingConstraints(comp(), _isGlobalPropagation); cur = cur->getNext()) {
        ...
        if (rel->relative == AbsoluteConstraint) {
            ...
            TR_ASSERT(cur->constraint->asRelation(), "assertion failure");
            TR_ASSERT(rel->constraint->asRelation(), "assertion failure");
            c = cur->constraint->asRelation()->propagateRelativeConstraint(rel->constraint->asRelation(), cur->relative,
                rel->relative, this);
            if (c) {
    ...
    if (rel->relative != AbsoluteConstraint) {
        TR_ASSERT(rel->constraint->asRelation(), "assertion failure");
        if (!addConstraintToList(node, rel->relative, valueNumber, rel->constraint->asRelation()->getComplement(this),
                valueConstraints))
            return false;
    }
...
//
TR::VPConstraint *OMR::ValuePropagation::addConstraintToList(TR::Node *node, int32_t valueNumber, int32_t relative,
    TR::VPConstraint *constraint, ValueConstraints *valueConstraints, bool replaceExisting)
{
    ...
    if (!existingGlobalConstraint) {
        ...
        for (rel = cur->relationships.getFirst(), prevRel = NULL; rel && rel->relative < relative;
             prevRel = rel, rel = rel->getNext()) {
            numRelatives++;
        }
        ...
        if (!rel && numRelatives > maxRelDepth) {
            _reachedMaxRelationDepth = true;
            logprintf(trace(), log, "===>Reached Max Relational Propagation Depth: %d\n", numRelatives);
        }
    ...
    if (cur) {
        for (store = cur->storeRelationships.getFirst(); store; store = store->getNext()) {
            ...
            if (node && node->getOpCode().hasSymbolReference() && (node->getSymbol() != storeSym))
                updateStoreConstraint = false;

            if (updateStoreConstraint) {
                for (storeRel = store->relationships.getFirst(), prev = NULL; storeRel;
                     prev = storeRel, storeRel = storeRel->getNext()) {
                ...
                if (!storeRel) {
                    ...
                    if (TR::VPConstraint::isSpecialClass((uintptr_t)constraint->getClass())) {
                        ...
                        log->prints("found special class constraint!\n");
                        // remove the type information
                        storeConstraint = TR::VPClass::create(this, NULL, constraint->getClassPresence(),
                            constraint->getPreexistence(), constraint->getArrayInfo(), constraint->getObjectLocation());
                    } else
                ...
                if (storeConstraint) {
                    if (storeConstraint != storeRel->constraint) {
                        ...
                        if (trace() && node) {
                            log->printf("   %s [%p] gets new store constraint:", node->getOpCode().getName(), node);
                            storeRel->print(log, this, valueNumber, 1);
                        }
                    }
                } else {
    ...
    if (newOrChanged) {
        if (trace() && node) {
            log->printf("   n%in %s gets new constraint:", node->getGlobalIndex(), node->getOpCode().getName());
            rel->print(log, this, valueNumber, 1);
            log->printf("type of constraint - longConstraint: %p, intConstraint: %p, shortConstraint: %p\n",
                rel->constraint->asLongConstraint(), rel->constraint->asIntConstraint(),
                rel->constraint->asShortConstraint());
        }
...
TR::VPConstraint *OMR::ValuePropagation::addGlobalConstraint(TR::Node *node, TR::VPConstraint *constraint,
    TR::Node *relative)
{
...
TR::VPConstraint *OMR::ValuePropagation::addGlobalConstraint(TR::Node *node, int32_t valueNumber,
    TR::VPConstraint *constraint, int32_t relative)
{
    ...
    for (rel = gc->constraints.getFirst(), prev = NULL; rel && rel->relative < relative;
    ...
    if (trace() && c == NULL)
        log->printf("Cannot intersect constraints on %s [%p]", node->getOpCode().getName(), node);
    // The assume below is informational in nature and should therefore not be triggered if production builds include
    // assumes (as on WCode or binopt). This condition should rarely show up in WCode or binopt (as Simplifier
    // transformations will homogenize the types) but can happen, for example, when an aggregate symbol is referenced in
    // one place on an oload and another as an iload. MG: Update assert?
    ...
    if (newOrChanged) {
        if (trace() && node) {
            log->printf("   %s [%p] gets new global constraint:", node->getOpCode().getName(), node);
            rel->print(log, this, valueNumber, 1);
        }
        ...
        if (trace() && node) {
            log->printf("   %s [%p] found existing global constraint value number %d (%p): ",
                node->getOpCode().getName(), node, valueNumber, c);
            c->print(log, comp());
            log->println();
        }
    }
...
TR::VPConstraint *OMR::ValuePropagation::addEdgeConstraint(TR::Node *node, TR::VPConstraint *constraint,
    EdgeConstraints *constraints, TR::Node *relative)
{
    if (!_isGlobalPropagation)
        return constraint;
...
TR::VPConstraint *OMR::ValuePropagation::addBlockConstraint(TR::Node *node, TR::VPConstraint *constraint,
    TR::Node *relative, bool mustBeValid)
{
    if (!constraint)
        return NULL;
    int32_t valueNumber = getValueNumber(node);
...
TR::VPConstraint *OMR::ValuePropagation::addBlockOrGlobalConstraint(TR::Node *node, TR::VPConstraint *constraint,
    bool isGlobal, TR::Node *relative)
{
    if (isGlobal)
...
{
    OMR::Logger *log = comp()->log();
    logprintf(trace(), log, "Merging relationships for value number: %i\n", valueNumber);
    ...
    while (from || to) {
        if (!to || (from && from->relative < to->relative)) {
            ...
            //
            if (preserveStoreRelationships && mergingStore && (from->relative == AbsoluteConstraint)
                && (!inBothLists /* || !isToEmpty */)) // commented out the !isToEmpty because it is not guaranteed to
                                                       // be safe to take an absolute constraint that only exists on one
                                                       // side
            {
        ...
        if (!from || from->relative > to->relative) {
            ...
            //
            rel = to->getNext();
            if (!preserveStoreRelationships || !mergingStore || (to->relative != AbsoluteConstraint)
        ...
        if (trace()) {
            log->prints("Attempting merge from: ");
            from->print(log, this);
            log->prints("\n           merge To: ");
            to->print(log, this);
            log->println();
        ...
        if (newConstraint) {
            ...
            if (global && global->relative == to->relative && global->constraint == newConstraint)
                newConstraint = NULL;
        }
...
void OMR::ValuePropagation::mergeStoreRelationships(ValueConstraint *fromvc, ValueConstraint *tovc, bool preserveFrom)
{
    ...
    while (from || to) {
        ...
        if (!fromRel) {
            ...
            //
            mergeRelationships(*fromRel, *toRel, valueNumber, preserve, prev /*mergingStore*/, &storeSymbols,
                inBothLists);
        }
    }
}
...
void OMR::ValuePropagation::mergeEdgeConstraints(EdgeConstraints *fromEdge, EdgeConstraints *toEdge)
{
    ...
    while (from || to) {
        if (!to || (from && from->getValueNumber() < to->getValueNumber())) {
            // The "from" value does not exist in the "to" list.
...
//
OMR::ValuePropagation::GlobalConstraint *OMR::ValuePropagation::GlobalConstraint::create(TR::Compilation *c,
    int32_t valueNumber)
{
...
//
inline uint32_t OMR::ValuePropagation::hashGlobalConstraint(int32_t valueNumber)
{
...
//
OMR::ValuePropagation::EdgeConstraints *OMR::ValuePropagation::EdgeConstraints::create(TR::Compilation *c,
    TR::CFGEdge *edge)
{
...
OMR::ValuePropagation::EdgeConstraints *OMR::ValuePropagation::createEdgeConstraints(TR::CFGEdge *edge,
    bool keepBlockList)
{
    if (!_isGlobalPropagation)
        return NULL;
...
//
static bool reduceCostOfAnalysisByTreatingBlockConservatively(TR::Block *b) { return b->isOSRCatchBlock(); }

void OMR::ValuePropagation::createExceptionEdgeConstraints(uint32_t exceptions, ValueConstraint *extraConstraint,
    TR::Node *reason)
{
    if (!_isGlobalPropagation)
        return;

    for (auto edge = _curBlock->getExceptionSuccessors().begin(); edge != _curBlock->getExceptionSuccessors().end();
         ++edge) {
        TR::Block *target = toBlock((*edge)->getTo());
        if (!target->canCatchExceptions(exceptions))
            continue;

        logprintf(trace(), comp()->log(), "   %s [%p] can throw exception to block_%d\n", reason->getOpCode().getName(),
        ...
        if (isUnreachablePath(constraints)) {
            ...
            for (vc = iter.getFirst(); vc; vc = iter.getNext()) {
                if (!vc->storeRelationships.isEmpty()) {
                    ValueConstraint *edgeVc
                        = _vcHandler.findOrCreate(vc->getValueNumber(), constraints->valueConstraints);
                    mergeStoreRelationships(vc, edgeVc, true);
                }
            }
        }
...
OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::findStoreRelationship(
    TR_LinkHead<StoreRelationship> &list, TR::Symbol *symbol)
{
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::findConstraintInList(TR_LinkHead<Relationship> &list,
    int32_t relative)
{
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::findValueConstraint(int32_t valueNumber,
    ValueConstraints &valueConstraints, int32_t relative)
{
    // Find the constraint for the given value number in the given value
    // constraint list.
    //
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::findEdgeConstraint(int32_t valueNumber,
    EdgeConstraints *edge, int32_t relative)
{
    return findValueConstraint(valueNumber, edge->valueConstraints, relative);
}

OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::findStoreValueConstraint(int32_t valueNumber,
    TR::Symbol *symbol, ValueConstraints &valueConstraints)
{
...
OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::findStoreConstraint(int32_t valueNumber,
    TR::Symbol *symbol)
{
    return findStoreValueConstraint(valueNumber, symbol, _curConstraints);
}

OMR::ValuePropagation::StoreRelationship *OMR::ValuePropagation::findStoreEdgeConstraint(int32_t valueNumber,
    TR::Symbol *symbol, EdgeConstraints *edge)
{
...
TR::VPConstraint *OMR::ValuePropagation::applyGlobalConstraints(TR::Node *node, int32_t valueNumber,
    TR::VPConstraint *constraint, int32_t relative)
{
    ...
    for (rel = gc->constraints.getFirst(); rel; rel = rel->getNext()) {
        ...
        if (reverse && follow) {
            ...
            if (relative == AbsoluteConstraint)
                linkedConstraint = reverse->constraint->asRelation()->propagateAbsoluteConstraint(follow->constraint,
                    valueNumber, this);
            else
                linkedConstraint = reverse->constraint->asRelation()->propagateRelativeConstraint(
                    follow->constraint->asRelation(), valueNumber, relative, this);
...
void OMR::ValuePropagation::createStoreConstraints(TR::Node *node)
{
    ...
    for (store = vc->storeRelationships.getFirst(), prev = NULL; store && store->symbol < symbol;
         prev = store, store = store->getNext()) {
    }
...
bool OMR::ValuePropagation::isDefInUnreachableBlock(int32_t defIndex)
{
    ...
    TR_StructureSubGraphNode *defSubGraphNode = comp()->getFlowGraph()->getStructure()->findNodeInHierarchy(
        defBlock->getParentStructureIfExists(comp()->getFlowGraph()), defBlock->getNumber());
    ...
    while (defSubGraphNode) {
        ...
        if (!reachableInputEdgesFound && defSubGraphNode->getStructure()->getParent()
            && (defSubGraphNode->getStructure()->getParent()->getNumber() == defSubGraphNode->getNumber()))
            defSubGraphNode = comp()->getFlowGraph()->getStructure()->findNodeInHierarchy(
                defSubGraphNode->getStructure()->getParent()->getParent(), defBlock->getNumber());
        else
    ...
    if (arrayRef == objectRef) {
        ...
        //
            ...
            if (sig && sig[0] == '[') {
                ...
                //
                if (len == 19 && array->isFixedClass() && !strncmp(sig, "[Ljava/lang/Object;", 19)) {
                    isNeeded = false;
                }
                ...
                //
                    ...
                    if (arrayComponentClass) {
                        isInstance = fe()->isInstanceOf(objectClass, arrayComponentClass, object->isFixedClass(),
                            array->isFixedClass());
                    }
                    ...
                    if (isInstance == TR_yes) {
                        ...
                        registerPreXClass(object);
                        mustFail = true;
                    } else if (arrayComponentClass && objectClass
                        && !TR::Compiler->cls.isClassArray(comp(), arrayComponentClass)
                    ...
                    else if (!comp()->compileRelocatableCode() && !(comp()->getOption(TR_DisableArrayStoreCheckOpts))
                        && arrayComponentClass && objectClass
...
void OMR::ValuePropagation::getArrayLengthLimits(TR::VPConstraint *constraint, int32_t &lowerBoundLimit,
    int32_t &upperBoundLimit, int32_t &elementSize, bool &isKnownObj)
{
    lowerBoundLimit = 0;
...
{
    OMR::Logger *log = comp()->log();
    logprints(trace(), log, "   checking for relationship between types...\n");
    ...
    if (lhs->asClass() && rhs->asClass()) {
        ...
        if (jlKlass && noPresenceResult) {
            logprints(trace(), log, "presences are incompatible\n");
            result = 0;
        } else if (jlKlass && noTypeResult) {
            logprints(trace(), log, "types are incompatible\n");
            result = 0;

            // if the object is a classobject, then the cast maybe one of the
            // interfaces implemented by Class
            if (isInstanceOf || isCheckCast) {
                if ((objectConstraint->isClassObject() == TR_yes) && (castIsClassObject == TR_maybe)) {
                    logprints(trace(), log, "object is a classobject but cast maybe Class\n");
                    result = value;
                }
            }
        } else if (isInstanceOf || isCheckCast) {
            if (!objectClass->getClassType() && (castIsClassObject == TR_no)
                && (isInstanceOf || objectClass->isNonNullObject()) && (objectClass->isClassObject() == TR_yes)) {
                logprints(trace(), log, "object is a classobject but cast is not a Class\n");
                result = 0;
            }
            /*
            ...
            */
            else if ((castIsClassObject == TR_yes) && !objectClass->getClassType()
                && (isInstanceOf || objectClass->isNonNullObject()) && (objectClass->isClassObject() == TR_no)) {
                result = 0;
                logprints(trace(), log, "object is not a classobject but cast is java/lang/Class\n");
            }
            // probably cannot get here
            //
            else if ((castIsClassObject == TR_yes) && !objectClass->getClassType()
                && (objectClass->isNonNullObject() || !isInstanceOf) && (objectClass->isClassObject() == TR_yes)) {
                result = 1;
                logprints(trace(), log, "object is a non-null classobject and cast is java/lang/Class\n");
            }
        }
...
void OMR::ValuePropagation::invalidateParmConstraintsIfNeeded(TR::Node *node, TR::VPConstraint *constraint)
{
    ...
    if (symRef && symRef->getSymbol()->getParmSymbol()) {
        ...
        if (parmConstraint) {
            logprintf(trace(), log, "Checking compatibility of store node %p parm %d with value\n", node, parmNum);
            int32_t result = 1;
    ...
    bool forceMerge)
{
    ...
    if (_defMergedNodes->get(node->getGlobalIndex())) {
        logprintf(trace(), log, "Node n%dn has already been processed by mergeDefConstraints - returning NULL\n",
            node->getGlobalIndex());
        return NULL;
    }
    ...
    if (!forceMerge && defNode) {
        logprintf(trace(), log, "   %s [%p] has singleDefiningLoad [%p] ", node->getOpCode().getName(), node, defNode);
        if (defNode->getVisitCount() >= _visitCount) {
            logprints(trace(), log, "  whose constraints already computed\n");
            return NULL;
        }
        logprints(trace(), log, "  going to merge\n");
    }
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
            logprintf(trace(), log, "   %s [%p] looking at method entry def point: ", node->getOpCode().getName(),
                node);
            ...
            if (relative == AbsoluteConstraint) {
                ...
                if (symRef && symRef->getSymbol()->getParmSymbol() && _parmValues
                    && isParmInvariant(symRef->getSymbol())) {
                    int32_t parmNum = symRef->getSymbol()->getParmSymbol()->getOrdinal();
            ...
            if (node->getSymbol()->isAutoOrParm() && _curDefinedOnAllPaths
                && !_curDefinedOnAllPaths->get(node->getSymbolReference()->getReferenceNumber())) {
                logprintf(trace(), log, "symRef %d is not stored on all paths - treating as unknown\n",
            ...
            // value unconstrained.
            //
            logprintf(trace(), log, "   %s [%p] looking at def value %d, %s [%p]: ", node->getOpCode().getName(), node,
                getValueNumber(defNode), defNode->getOpCode().getName(), defNode);
                    ...
                    if (defValueConstraint) {
                        ...
                        if (rel) {
                            if (rel->constraint->asUnreachablePath()) {
                                ...
                                //
                                logprints(trace(), log, "def is unreachable (constraint), ignored\n");
                                continue;
                            }
                            isGlobal = false;
                        } else {
                        ...
                        if (!isDefInUnreachableBlock(defIndex)) {
                            ...
                            logprints(trace(), log, "def is unreachable (defInUnreachableBlock), ignored\n");
                            continue;
                        }
#if 0
        ...
        if (defIsUnseen) {
            logprints(trace(), log, "def has not been seen yet\n");
        } else if (!defConstraint) {
            logprints(trace(), log, "no constraint\n");
            constraint = NULL;
            unconstrained = true;
        } else {
            if (trace()) {
            ...
            if (defNode && node) {
                if (defNode->getOpCode().isStoreDirect()) {
                    ...
                    if ((firstChild->getOpCodeValue() == TR::acall) || (firstChild->getOpCodeValue() == TR::acalli)) {
            ...
            if (mergeConstraint) {
                logprintf(trace(), log, " merging constraint at def %p\n", defNode);
                if (constraint) {
                ...
                logprintf(trace(), log, " NOT merging constraint at def %p\n", defNode);
        }
    }
    ...
    if (!bigDecimalConstraints.isEmpty()) {
        ...
        if (constraint && mergeDeferredConstraints) {
            ...
            for (c = cIt.getFirst(); c; c = cIt.getNext()) {
                logprintf(trace(), log, " doing late merging constraint at def %p\n", defNode);
    ...
    if (relative != AbsoluteConstraint) {
        logprints(trace(), log,
            "we are processing a relative constraint and merging with backedge constraints is not supported\n");
        return NULL;
    }
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound
            && !_curDefinedOnAllPaths->get(defNode->getSymbolReference()->getReferenceNumber())) {
            logprintf(trace(), log, "symref %d is not defined on all paths - consulting backedge constraints\n",
            ...
            for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent) {
                logprintf(trace(), log,
                    "   %s [%p] looking at back edges in loop %d for def %s [%p]: ", node->getOpCode().getName(), node,
                    loopInfo->_loop->getNumber(), defNode->getOpCode().getName(), defNode);

                // See if the unseen def was seen on a path to this back edge.
                //
                if (!loopInfo || !loopInfo->_backEdgeConstraints) {
                    logprints(trace(), log, "not seen on this back edge, ignored\n");
                    continue;
                }

                if (!hasBeenStored(defValueNumber, sym, loopInfo->_backEdgeConstraints->valueConstraints)) {
                    logprints(trace(), log, "no backedge constraint, ignored\n");
                    continue;
                }
                ...
                if (defConstraint && defConstraint->asEqual() && defConstraint->asEqual()->increment() == 0) {
                    logprints(trace(), log, "trivially equal, ignored\n");
                    continue;
                }
                ...
                if (defConstraint->asUnreachablePath()) {
                    logprints(trace(), log, "def is unreachable, ignored\n");
                    continue;
                }
                                ...
                                if (method
                                    ...
                                    if ((thisChild == node)
                                        ...
                                        if (thisChild != node) {
                                            ...
                                            TR_UseDefInfo::BitVector thisDefs(comp()->allocator());
                                            if (!_useDefInfo->getUseDef(thisDefs, thisUseDefIndex)
                                                || (!(thisDefs == defs)))
                                                avoidMerge = true;
                                        }
                ...
                if (mergeConstraint) {
                    logprintf(trace(), log, " mmerging constraint at def %p\n", defNode);
                    if (constraint) {
                        //
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint32_t)defConstraint->getHighInt() < TR::getMaxUnsigned<TR::Int32>()) {
                                        if ((uint32_t)defConstraint->getLowInt() < (uint32_t)constraint->getLowInt()
                                            && (uint32_t)constraint->getLowInt()
                                                <= unsigned(defConstraint->getHighInt())) {
                                            TR::VPConstraint *newDefConstraint = TR::VPIntRange::create(this,
                                ...
                                if (defConstraint->getLowInt() > TR::getMinSigned<TR::Int32>()) {
                                    if (defConstraint->getLowInt() <= constraint->getHighInt()
                                        ...
                                        TR::VPConstraint *newDefConstraint = TR::VPIntRange::create(this,
                                            defConstraint->getLowInt(), constraint->getHighInt());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint16_t)defConstraint->getHighShort() < TR::getMaxUnsigned<TR::Int16>()) {
                                        if ((uint16_t)defConstraint->getLowShort() < (uint16_t)constraint->getLowShort()
                                            ...
                                            TR::VPConstraint *newDefConstraint = TR::VPShortRange::create(this,
                                ...
                                if (defConstraint->getLowShort() > TR::getMinSigned<TR::Int16>()) {
                                    if (defConstraint->getLowShort() <= constraint->getHighShort()
                                        ...
                                        TR::VPConstraint *newDefConstraint = TR::VPShortRange::create(this,
                                            defConstraint->getLowShort(), constraint->getHighShort());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint64_t)defConstraint->getHighLong() < TR::getMaxUnsigned<TR::Int64>()) {
                                        if ((uint64_t)defConstraint->getLowLong() < (uint64_t)constraint->getLowLong()
                                            && (uint64_t)constraint->getLowLong()
                                                <= (uint64_t)defConstraint->getHighLong()) {
                                            TR::VPConstraint *newDefConstraint = TR::VPLongRange::create(this,
                                ...
                                if (defConstraint->getLowLong() > TR::getMinSigned<TR::Int64>()) {
                                    if (defConstraint->getLowLong() <= constraint->getHighLong()
                                        ...
                                        TR::VPConstraint *newDefConstraint = TR::VPLongRange::create(this,
                                            defConstraint->getLowLong(), constraint->getHighLong());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                            }
                    ...
                    logprintf(trace(), log, " NOT mmerging constraint at def %p\n", defNode);
            }
        }
    ...
    if (!bigDecimalConstraints.isEmpty()) {
        ...
        if (constraint && mergeDeferredConstraints) {
            ...
            TR::VPConstraint *c;
            for (c = cIt.getFirst(); c; c = cIt.getNext()) {
                logprintf(trace(), log, " late mmerging constraint at def %p\n", defNode);
                constraint = constraint->merge(c, this);
                if (!constraint) {
                    unconstrained = true;
                    break;
...
void OMR::ValuePropagation::removeRestOfBlock()
{
    ...
    for (treeTop = _curTree->getNextTreeTop(); treeTop->getNode()->getOpCodeValue() != TR::BBEnd; treeTop = next) {
...
void OMR::ValuePropagation::mustTakeException()
{
    ...
    if (!performTransformation(comp(), "%sRemoving rest of block after %s [%p]\n", OPT_DETAILS,
            _curTree->getNode()->getOpCode().getName(), _curTree->getNode()))
        return;
...
bool OMR::ValuePropagation::isHighWordZero(TR::Node *node)
{
    ...
    if (constraint) {
        ...
        if (longConstraint && (longConstraint->getLow() >= 0)
...
void OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node *node)
{
    ...
    logprintf(trace(), log, "   %s [%p] may be induction variable [%p]\n", node->getOpCode().getName(), node, sym);
        ...
        if (!_useDefInfo->getUseDef(defs, useIndex))
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
                    ...
                    //
                    isInductionVariable = false;
                    logprintf(trace(), log, "      ivInfo is invalid for sym [%p] as method entry is def point\n", sym);
                    break;
                }
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if ((defVN != incrementVN) && !notInLoop) {
                    ...
                    isInductionVariable = false;
                    logprintf(trace(), log,
                        "      ivInfo is invalid for sym [%p], def [%d] node %p does not have the same value number as "
                        "another increment\n",
                        sym, defVN, defNode);
                    break;
                }
                ...
                if (entryVN >= 0) {
                    ...
                    if (_loopInfo->_seenDefs->get(defVN)) {
                        maybeInductionVariable = false;
                        isInductionVariable = false;
                        logprintf(trace(), log, "      ivInfo is invalid for sym [%p], def [%d] already seen in loop\n",
                            sym, defVN);
                        break;
                    } else {
                        if (trace()) {
                            log->printf("      Sym [%p] has more than two defs, extra def due to VN [%d]\n", sym,
                                entryVN);
                            log->prints("      Maybe able to guess the increment\n");
                        }
                ...
                if ((defVN != entryVN) && notInLoop) {
                    ...
                    isInductionVariable = false;
                    logprintf(trace(), log,
                        "      ivInfo is invalid for sym [%p], def [%d] node %p is likely in some inner loop\n", sym,
                        defVN, defNode);
                    break;
                }
            }
    ...
    if (iv) {
        ...
        if (maybeInductionVariable) {
            if (checkLoopTestBlock(sym)) {
                ...
                iv->_entryDef = NULL;
                logprintf(trace(), log, "      Guessed increment of the iv for sym [%p]\n", sym);
            } else
                logprintf(trace(), log,
                    "      Could not guess the increment for sym [%p], not marked as induction variable\n", sym);
        }
    ...
    //
    if (iv->_entryDef) {
        // select the right type of constraint for induction variables to prevent merging different types of constraints
        // together
        if (node->getOpCode().isLong())
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPLongConst::create(this, incrementVN),
                &_curConstraints, true);
        else if (node->getOpCode().isShort())
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPShortConst::create(this, incrementVN),
                &_curConstraints, true);
        else
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPIntConst::create(this, incrementVN),
                &_curConstraints, true);
    }
}
...
void OMR::ValuePropagation::checkForInductionVariableLoad(TR::Node *node)
{
    ...
    if (iv && iv->_entryDef) {
        ...
        // together
        if (node->getOpCode().isLong())
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                TR::VPLongConst::create(this, useValueNumber), &_curConstraints, true);
        else if (node->getOpCode().isShort())
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                TR::VPShortConst::create(this, useValueNumber), &_curConstraints, true);
        else
            addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                TR::VPIntConst::create(this, useValueNumber), &_curConstraints, true);
    }
}
...
bool OMR::ValuePropagation::checkLoopTestBlock(TR::Symbol *sym)
{
    ...
    if (!isWellFormedLoop) {
        ...
        for (TR_StructureSubGraphNode *sgNode = snIt.getCurrent(); sgNode && (branchNode == NULL);
             sgNode = snIt.getNext()) {
    ...
    if (isWellFormedLoop) {
        ...
        for (TR::CFGEdge *e = cfgIt.getFirst(); e; e = cfgIt.getNext()) {
            ...
            if (loopTestBlock) {
                ...
                if (branchNode->getOpCode().isBranch() && branchNode->getOpCode().isBooleanCompare()) {
...
void OMR::ValuePropagation::checkBackEdgeCoverage()
{
    ...
    for (iv = _loopInfo->_inductionVariables.getFirst(); iv; iv = next) {
        ...
        if (iv->_entryDef) {
            ...
            if (rel) {
                TR_ASSERT(
                    ((rel->constraint->asIntConst() && rel->constraint->asIntConst()->getInt() == iv->_incrementVN)
                        || (rel->constraint->asLongConst()
                            && rel->constraint->asLongConst()->getLong() == iv->_incrementVN)
                        || (rel->constraint->asShortConst()
                            && rel->constraint->asShortConst()->getShort() == iv->_incrementVN)),
...
void OMR::ValuePropagation::setUpInductionVariables(TR_StructureSubGraphNode *node)
{
    ...
    while ((ivInfo = _loopInfo->_inductionVariables.pop())) {
        ...
        if (ivInfo->_symbol->getType().isInt32()) {
            if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
                if (ivInfo->_entryConstraint->asIntConst())
                    entry = new (trHeapMemory()) TR::VPIntConst(ivInfo->_entryConstraint->getLowInt());
                else
                    entry = new (trHeapMemory())
                        TR::VPIntRange(ivInfo->_entryConstraint->getLowInt(), ivInfo->_entryConstraint->getHighInt());
            } else
                entry = NULL;
            if (ivInfo->_increment->asIntConst())
                incr = new (trHeapMemory()) TR::VPIntConst(ivInfo->_increment->getLowInt());
            else
                incr = new (trHeapMemory())
                    TR::VPIntRange(ivInfo->_increment->getLowInt(), ivInfo->_increment->getHighInt());
            if (exitConstraint) {
                if (exitConstraint->asIntConst())
                    exit = new (trHeapMemory()) TR::VPIntConst(exitConstraint->getLowInt());
                else
                    exit = new (trHeapMemory())
                        TR::VPIntRange(exitConstraint->getLowInt(), exitConstraint->getHighInt());
            } else
                exit = NULL;
        } else if (ivInfo->_symbol->getType().isInt16()) {
            if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
                if (ivInfo->_entryConstraint->asShortConst())
                    ...
                    entry = new (trHeapMemory()) TR::VPShortRange(ivInfo->_entryConstraint->getLowShort(),
                        ivInfo->_entryConstraint->getHighShort());
            } else
                entry = NULL;
            if (ivInfo->_increment->asShortConst())
                ...
                incr = new (trHeapMemory())
                    TR::VPShortRange(ivInfo->_increment->getLowShort(), ivInfo->_increment->getHighShort());
            if (exitConstraint) {
                if (exitConstraint->asShortConst())
                    exit = new (trHeapMemory()) TR::VPShortConst(exitConstraint->getLowShort());
                else
                    exit = new (trHeapMemory())
                        TR::VPShortRange(exitConstraint->getLowShort(), exitConstraint->getHighShort());
            } else
                exit = NULL;
            ...
            if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
                if (ivInfo->_entryConstraint->asLongConst())
                    ...
                    entry = new (trHeapMemory()) TR::VPLongRange(ivInfo->_entryConstraint->getLowLong(),
                        ivInfo->_entryConstraint->getHighLong());
            } else
                entry = NULL;
            if (ivInfo->_increment->asLongConst())
                ...
                incr = new (trHeapMemory())
                    TR::VPLongRange(ivInfo->_increment->getLowLong(), ivInfo->_increment->getHighLong());
            if (exitConstraint) {
                if (exitConstraint->asLongConst())
                    exit = new (trHeapMemory()) TR::VPLongConst(exitConstraint->getLowLong());
                else
                    exit = new (trHeapMemory())
                        TR::VPLongRange(exitConstraint->getLowLong(), exitConstraint->getHighLong());
            } else
                exit = NULL;
        }
        ...
        TR_InductionVariable *iv = new (trHeapMemory())
        ...
        if (trace()) {
            ...
            if (ivInfo->_entryConstraint && ivInfo->_entryDef) {
                log->prints("\n   Entry constraint : ");
                ivInfo->_entryConstraint->print(log, comp());
            }
            log->prints("\n   Increment constraint : ");
            ivInfo->_increment->print(log, comp());
            if (exitConstraint) {
                log->prints("\n   Exit constraint : ");
                exit->print(log, comp());
            }
...
void OMR::ValuePropagation::collectBackEdgeConstraints()
{
    ...
    for (edge = pi.getFirst(); edge; edge = pi.getNext()) {
        ...
        if (_loopInfo->_backEdgeConstraints->valueConstraints.isEmpty()) {
            ...
            //
            _vcHandler.setRoot(_loopInfo->_backEdgeConstraints->valueConstraints,
                _vcHandler.getRoot(constraints->valueConstraints));
            _vcHandler.setRoot(constraints->valueConstraints, NULL);
        } else {
...
int32_t TR::GlobalValuePropagation::perform()
{
    ...
    if (skipBlocksThatCannotReachNonColdBlocks) {
        _blocksToProcess = new (trStackMemory()) TR_BitVector(comp()->getFlowGraph()->getNumberOfNodes(),
            comp()->trMemory(), stackAlloc, notGrowable, TR_MemoryBase::ValuePropagation);
...
void OMR::ValuePropagation::getParmValues() { return; }
...
void TR::GlobalValuePropagation::propagateOutputConstraints(TR_StructureSubGraphNode *node, bool lastTimeThrough,
    bool isNaturalLoop, List<TR::CFGEdge> &outEdges1, List<TR::CFGEdge> *outEdges2)
{
        ...
        for (edge = exits.getFirst(); edge; edge = exits.getNext()) {
            if (edge->getTo()->getNumber() == parentTo) {
                ...
                if (isUnreachablePath(parentConstraints)) {
                    ...
                    _vcHandler.setRoot(parentConstraints->valueConstraints,
                        _vcHandler.getRoot(constraints->valueConstraints));
                    _vcHandler.setRoot(constraints->valueConstraints, NULL);
                } else {
...
void TR::GlobalValuePropagation::processStructure(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
    ...
    if (comp()->getOptions()->realTimeGC() && ((++numIter) & 0xf) == 0
    ...
    if (region) {
        ...
        TR_ASSERT_FATAL(!node->getStructure()->asBlock()->getBlock()->isExtensionOfPreviousBlock(),
            "This optimization does not run on extended blocks");
        _defMergedNodes->empty();
        processBlock(node, lastTimeThrough, insideLoop);
    }
}

void TR::GlobalValuePropagation::processAcyclicRegion(TR_StructureSubGraphNode *node, bool lastTimeThrough,
    bool insideLoop)
{
    if (trace())
    ...
    bool insideLoop)
{
    ...
    if (!insideLoop) {
        ...
        //
        if (trace())
            printStructureInfo(log, region, true, false);
        ...
        for (auto itr = region->getEntry()->getPredecessors().begin(),
                  end = region->getEntry()->getPredecessors().end();
             itr != end; ++itr) {
        ...
        if (trace())
            printStructureInfo(log, region, false, false);
        ...
        if (!lastTimeThrough) {
            ...
            //
            for (_loopInfo = parentLoopInfo->_subLoops.getFirst(); _loopInfo && _loopInfo->_loop != region;
    ...
    if (trace())
        printStructureInfo(log, region, true, lastTimeThrough);
    ...
    for (auto itr = region->getEntry()->getPredecessors().begin(), end = region->getEntry()->getPredecessors().end();
         itr != end; ++itr) {
        (*_definedOnAllPaths)[*itr] = NULL;
    }
    ...
    if (trace()) {
        log->printf("   defined on all paths for entry of loop %d", region->getNumber());
        inboundDefinedOnAllPaths->print(log, comp());
        log->println();
    }

    ListIterator<TR::CFGEdge> it(&region->getExitEdges());
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
        logprintf(trace(), log, "   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(),
            edge->getTo()->getNumber());

        if ((*_definedOnAllPaths)[edge]) {
            if (trace()) {
                ((*_definedOnAllPaths)[edge])->print(log, comp());
                log->println();
            }
            (*(*_definedOnAllPaths)[edge]) |= *inboundDefinedOnAllPaths;
        } else {
            logprints(trace(), log, " NULL\n");
            (*_definedOnAllPaths)[edge] = inboundDefinedOnAllPaths;
        }
    ...
    if (trace())
        printStructureInfo(log, node->getStructure(), false, lastTimeThrough);
...
void OMR::ValuePropagation::findStoresInBlock(TR::Block *block, ValueConstraints &stores)
{
    ...
    for (treeTop = block->getEntry(); treeTop && treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStore()) {
            ...
            for (rel = vc->storeRelationships.getFirst(), prev = NULL; rel && rel->symbol < node->getSymbol();
                 prev = rel, rel = rel->getNext()) {
            }
...
void TR::GlobalValuePropagation::processRegionSubgraph(TR_StructureSubGraphNode *node, bool lastTimeThrough,
    bool insideLoop, bool isNaturalLoop)
{
...
void TR::GlobalValuePropagation::processRegionNode(TR_StructureSubGraphNode *node, bool lastTimeThrough,
    bool insideLoop)
{
    ...
    for (auto itr = node->getPredecessors().begin(), end = node->getPredecessors().end(); itr != end; ++itr) {
        ...
        if (trace()) {
            ...
            if (predDefinedOnAllPaths)
                predDefinedOnAllPaths->print(log, comp());
            else
                log->prints("NULL");
            log->println();
        }
...
void TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop)
{
    ...
    logprintf(trace(), log, "GVP: Processing block_%i\n", _curBlock->getNumber());
    ...
    if (insideLoop && trace()) {
        log->printf("   defined on all paths for entry of block %d", block->getNumber());
        _curDefinedOnAllPaths->print(log, comp());
        log->println();
    }
    ...
    if (comp()->getStartBlock()->getNumber() == _curBlock->getNumber()) {
        addConstraintToList(NULL, _syncValueNumber, AbsoluteConstraint, TR::VPSync::create(this, TR_no),
            &_curConstraints);
    }

    if (trace())
        printStructureInfo(log, node->getStructure(), true, lastTimeThrough);
    ...
    if (insideLoop) {
        for (auto itr = node->getSuccessors().begin(), end = node->getSuccessors().end(); itr != end; ++itr) {
            if (trace()) {
                log->printf("   outbound seenOnAllpaths for edge %d->%d", (*itr)->getFrom()->getNumber(),
                    (*itr)->getTo()->getNumber());
                _curDefinedOnAllPaths->print(log, comp());
                log->println();
            }
    ...
    if (!isUnreachablePath(_curConstraints)) {
        ...
        //
        logprintf(trace(), log, "Calling findOutEdge using _curBlock %d and nextBlock %d\n", _curBlock->getNumber(),
            nextBlock->getNumber());

        edge = findOutEdge(_curBlock->getSuccessors(), nextBlock);
        logprintf(trace(), log, "Processing %s [%p]\n   Fall-through to next block\n",
            endTree->getNode()->getOpCode().getName(), endTree->getNode());

        OMR::ValuePropagation::EdgeConstraints *ec = createEdgeConstraints(edge, false);
        if (trace())
            printEdgeConstraints(log, ec);
    }
    ...
    // Create temporary List structures to pass to this function. Will be eliminated once getExitEdges has migrated to
    // STL
    ...
    for (auto iter = _curBlock->getSuccessors().begin(); iter != _curBlock->getSuccessors().end(); ++iter)
        List1.add(*iter);
    for (auto iter = _curBlock->getExceptionSuccessors().begin(); iter != _curBlock->getExceptionSuccessors().end();
         ++iter)
        List2.add(*iter);
...
{
    return !valueConstraints.isEmpty() && !_vcHandler.getRoot(valueConstraints)->relationships.isEmpty()
...
void OMR::ValuePropagation::printStructureInfo(OMR::Logger *log, TR_Structure *s, bool starting, bool lastTimeThrough)
{
    ...
    printParentStructure(log, s);
    log->printf("%d", s->getNumber());
    if (lastTimeThrough)
        log->prints(" last time through\n");
    else
        log->prints(" first time through\n");

    if (starting) {
        printGlobalConstraints(log);
        log->prints("   Starting edge constraints:\n");
        if (_curConstraints.isEmpty()) {
            log->prints("      NONE\n");
        } else {
            printValueConstraints(log, _curConstraints);
        }

        if (isLoop && lastTimeThrough) {
            log->prints("   Back edge constraints:\n");
            if (!_loopInfo->_backEdgeConstraints || _loopInfo->_backEdgeConstraints->valueConstraints.isEmpty()) {
                log->prints("      NONE\n");
            } else {
                printValueConstraints(log, _loopInfo->_backEdgeConstraints->valueConstraints);
            }
        }
    }
}

void OMR::ValuePropagation::printParentStructure(OMR::Logger *log, TR_Structure *s)
{
    if (s->getParent()) {
        printParentStructure(log, s->getParent());
        log->printf("%d->", s->getParent()->getNumber());
    }
}

void OMR::ValuePropagation::printValueConstraints(OMR::Logger *log, ValueConstraints &valueConstraints)
{
...
void OMR::ValuePropagation::printGlobalConstraints(OMR::Logger *log)
{
    log->prints("   Global constraints:\n");
    for (auto i = 0U; i <= _globalConstraintsHTMaxBucketIndex; i++) {
        ...
        for (entry = _globalConstraintsHashTable[i]; entry; entry = entry->next) {
            for (Relationship *rel = entry->constraints.getFirst(); rel; rel = rel->getNext()) {
                log->prints("      global");
                rel->print(log, this, entry->valueNumber, 1);
            }
        }
    }
}

void OMR::ValuePropagation::printEdgeConstraints(OMR::Logger *log, EdgeConstraints *constraints)
{
    if (!_isGlobalPropagation)
        return;
    ...
    if (trace()) {
        ...
        log->printf("   Edge %d->%d", from->getNumber(), to->getNumber());
        if (isUnreachablePath(constraints)) {
            log->prints(" is unreachable\n");
        } else if (constraints->valueConstraints.isEmpty()) {
            log->prints(" has no constraints\n");
        } else {
            log->prints(" constraints:\n");
            printValueConstraints(log, constraints->valueConstraints);
        }
    }
}

void OMR::ValuePropagation::Relationship::print(OMR::Logger *log, OMR::ValuePropagation *vp)
{
    if (relative == AbsoluteConstraint) {
        ...
        if (constraint)
            ...
            log->prints("generalized");
    } else
        constraint->print(log, vp->comp(), relative);
}

void OMR::ValuePropagation::Relationship::print(OMR::Logger *log, OMR::ValuePropagation *vp, int32_t valueNumber,
    int32_t indent)
{
    TR_FrontEnd *fe = vp->fe();
    if (valueNumber < vp->_firstUnresolvedSymbolValueNumber) {
        log->printf("%*.svalue %d is ", indent, " ", valueNumber);
        print(log, vp);
    } else if (valueNumber < vp->_firstInductionVariableValueNumber) {
        // Symbol resolution constraint
        //
        log->printf("%*.ssymbol %d is resolved", indent, " ", valueNumber - vp->_firstUnresolvedSymbolValueNumber);
    } else {
        ...
        if (iv)
            log->printf("%*.sinduction variable %d [%p]", indent, " ",
                valueNumber - vp->_firstInductionVariableValueNumber, iv->_symbol);
        else
...
void OMR::ValuePropagation::StoreRelationship::print(OMR::Logger *log, OMR::ValuePropagation *vp, int32_t valueNumber,
    int32_t indent)
{
    if (relationships.getFirst()) {
        for (Relationship *rel = relationships.getFirst(); rel; rel = rel->getNext()) {
            log->printf("%*.ssymbol %p store", indent, " ", symbol);
            rel->print(log, vp, valueNumber, 1);
        }
    } else
        log->printf("%*.sptr %p symbol %p has no relationships\n", indent, " ", this, symbol);
}

void OMR::ValuePropagation::ValueConstraint::print(OMR::Logger *log, OMR::ValuePropagation *vp, int32_t indent)
{
    for (Relationship *rel = relationships.getFirst(); rel; rel = rel->getNext())
    ...
    for (StoreRelationship *storeRel = storeRelationships.getFirst(); storeRel; storeRel = storeRel->getNext())
        storeRel->print(log, vp, getValueNumber(), indent);
}
...
bool OMR::ValuePropagation::removeConstraints(int32_t valueNumber, ValueConstraints *vc, bool findStores)
{
    ...
    logprints(trace(), log, "   Setting fall-through as unreachable\n");
    // mark current X as unreachable
    // X = block
    if (findStores) {
        logprintf(trace(), log, "   block following block_%d is unreachable\n", _curBlock->getNumber());
        setUnreachablePath();
    }
...
{
    OMR::Logger *log = comp()->log();
    logprintf(trace(), log, "   Intersection of constraints failed for valueNumber [%d], removing constraints\n",
        valueNumber);
    ...
    if (vc) {
        ...
        for (rel = vc->relationships.getFirst(); rel; rel = next) {
            ...
            if (trace()) {
                log->prints("   removing absoulte constraint:\n");
                rel->print(log, this, valueNumber, 6);
            }
        ...
        for (curStore = vc->storeRelationships.getFirst(); curStore; curStore = nextStore) {
            ...
            for (rel = curStore->relationships.getFirst(); rel; rel = rel->getNext()) {
                ...
                if (trace()) {
                    log->prints("   removing absolute store constraint:\n");
                    rel->print(log, this, valueNumber, 6);
                }
            }
    ...
    if (vc) {
        ...
        for (curStore = vc->storeRelationships.getFirst(); curStore; curStore = curStore->getNext()) {
            ...
            for (rel = curStore->relationships.getFirst(); rel; rel = next) {
                ...
                if (rel->relative == relative) {
                    if (trace()) {
                        log->prints("   removing store relationship:\n");
                        rel->print(log, this, rel->relative, 6);
                    }
...
{
    OMR::Logger *log = comp()->log();
    logprintf(trace(), log, "   Intersection failed for value number [%d], removing global constraints\n", valueNumber);
    ...
    for (rel = gc->constraints.getFirst(); rel; rel = next) {
        ...
        if (rel->relative != AbsoluteConstraint) {
            ...
            if (relGC) {
                ...
                if (foundRel) {
                    if (trace()) {
                        log->prints("   removing global relationship:\n");
                        rel->print(log, this, cur->relative, 6);
                    }
        ...
        if (trace()) {
            log->prints("   removing global absolute constraint:\n");
            rel->print(log, this, valueNumber, 6);
        }
...
TR::TreeTop *TR::ArraycopyTransformation::createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef, bool isForward)
{
...
TR::TreeTop *TR::ArraycopyTransformation::createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::Node *len, TR::SymbolReference *srcObjRef,
    TR::SymbolReference *dstObjRef, bool isForward)
{
    OMR::Logger *log = comp()->log();
    TR::Node *root = tree->getNode()->getFirstChild();
    TR::Node *src = NULL;
    ...
    if (!root->isReferenceArrayCopy()) {
        ...
        logprintf(trace(), log, "Created 3-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
        ...
        logprintf(trace(), log, "Created 5-child arraycopy %s from root node %s, ", comp()->getDebug()->getName(node),
            comp()->getDebug()->getName(root));
    }
...
int64_t TR::ArraycopyTransformation::arraycopyHighFrequencySpecificLength(TR::Node *arrayCopyNode)
{
    ...
    if (comp()->getRecompilationInfo()) {
        if (comp()->target().is64Bit()) {
            TR_LongValueInfo *valueInfo = static_cast<TR_LongValueInfo *>(
                TR_ValueProfileInfoManager::getProfiledValueInfo(arrayCopyNode, comp(), LongValueInfo));
            ...
            TR_ValueInfo *valueInfo = static_cast<TR_ValueInfo *>(
                TR_ValueProfileInfoManager::getProfiledValueInfo(arrayCopyNode, comp(), ValueInfo));
            if (valueInfo && valueInfo->getTopProbability() > MIN_ARRAYCOPY_FREQ_FOR_SPECIALIZATION) {
                return ((int64_t)valueInfo->getTopValue());
            }
        }
...
TR::TreeTop *TR::ArraycopyTransformation::createPointerCompareNode(TR::Node *node, TR::SymbolReference *srcRef,
...
TR::TreeTop *TR::ArraycopyTransformation::createRangeCompareNode(TR::Node *node, TR::SymbolReference *srcRef,
    TR::SymbolReference *dstRef, TR::SymbolReference *lenRef)
{
    bool is64Bit = comp()->target().is64Bit();
    TR::Node *cmp;
    ...
    if (is64Bit) {
        if (len->getType().isInt32())
            srcEndNode = TR::Node::create(TR::aladd, 2, src, TR::Node::create(TR::i2l, 1, len));
        else
...
TR::TreeTop *TR::ArraycopyTransformation::createMultipleArrayNodes(TR::TreeTop *arrayTree, TR::Node *node)
{
    ...
    TR::TreeTop *firstInsertedTree
        = createStoresForArraycopyChildren(comp(), arrayTree, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
    ...
    if (!alreadyForwardArrayCopy) {
        ...
        followOnBlock = arraycopyBlock->createConditionalBlocksBeforeTree(arrayTree, compareTree, arraycopyBackward,
            outerElseTree, cfg);

        TR::Block *forwardArrayCopyBlock
            = TR::Block::createEmptyBlock(node, comp(), arraycopyBlock->getFrequency(), arraycopyBlock);
    ...
    if (specificLength >= 0)
        specializeForLength(outerArraycopyTree, node, (uintptr_t)specificLength, srcRef, dstRef, lenRef, srcObjRef,
            dstObjRef);
...
TR::TreeTop *TR::ArraycopyTransformation::tryToSpecializeForLength(TR::TreeTop *tt, TR::Node *arraycopyNode)
{
    ...
    int64_t specificLength
        = arraycopyHighFrequencySpecificLength(arraycopyNode); // TODO: What about 64-bit lengths on 64-bit platforms?
    if (specificLength >= 0
        && performTransformation(comp(), "%sSpecializing arraycopy %s for length of %d bytes\n", OPT_DETAILS,
            comp()->getDebug()->getName(arraycopyNode), specificLength)) {
        TR::SymbolReference *srcObjRef = NULL;
        TR::SymbolReference *dstObjRef = NULL;
        TR::SymbolReference *srcRef = NULL;
        TR::SymbolReference *dstRef = NULL;
        TR::SymbolReference *lenRef = NULL;
        TR::TreeTop *firstInsertedTree
            = createStoresForArraycopyChildren(comp(), tt, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
...
{
    TR::Node *node
        = comp->target().is64Bit() ? TR::Node::lconst(n, val) : TR::Node::iconst(n, static_cast<int32_t>(val));
    if (node->getOpCodeValue() == TR::lconst)
        node->setLongInt(val);
    return node;
}

TR::TreeTop *TR::ArraycopyTransformation::specializeForLength(TR::TreeTop *tt, TR::Node *arraycopyNode,
    uintptr_t lengthInBytes, TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef)
{
    ...
    TR::TreeTop *specializedTree = createArrayNode(tt, arraycopySpecific, srcRef, dstRef,
        addressSizedConst(comp(), arraycopyNode, lengthInBytes), srcObjRef, dstObjRef, true);
    if (trace())
        dumpOptDetails(comp(), "%s Specialized arraycopy is %s\n", OPT_DETAILS,
            getDebug()->getName(specializedTree->getNode()->getFirstChild()));

    TR::Node *lengthNode = TR::Node::createLoad(arraycopyNode, lenRef);

    TR::TreeTop *specificIfCheck = TR::TreeTop::create(comp(),
        TR::Node::createif(lengthNode->getType().isInt32() ? TR::ificmpne : TR::iflcmpne, lengthNode,
            addressSizedConst(comp(), arraycopyNode, lengthInBytes)));

    TR::Block *followOnBlock = tt->getEnclosingBlock()->createConditionalBlocksBeforeTree(tt, specificIfCheck,
        arraycopyOriginal, arraycopySpecific, comp()->getFlowGraph());
    ...
    requestOpt(OMR::basicBlockOrdering); // Block frequency is reset, so run order blocks optimization to reorder this
                                         // path appropriately.
...
int32_t TR::ArraycopyTransformation::perform()
{
    ...
    if (!hasChangedTrees()
        || !performTransformation(comp(), "%s Arraycopy Transformation for primitive and reference arrays\n",
            OPT_DETAILS))
        return 0;
...
void OMR::ValuePropagation::versionBlocks()
{
    ...
    for (cur = _blocksToBeVersioned->getFirst(); cur; cur = cur->getNext()) {
        ...
        for (ArrayLengthToVersion *arrayLength = cur->_arrayLengths->getFirst(); arrayLength;
             arrayLength = arrayLength->getNext()) {
            for (ArrayIndexInfo *arrayIndex = arrayLength->_arrayIndicesInfo->getFirst(); arrayIndex;
        ...
        while (1) {
            ...
            for (fromTT = fromTT->getNextTreeTop(); fromTT != tempBlock->getExit(); fromTT = fromTT->getNextTreeTop()) {
                if (fromTT->getNode()->getOpCode().isBranch())
                    fromTT->getNode()->setBranchDestination(
                        cloner.getToBlock(fromTT->getNode()->getBranchDestination()->getNode()->getBlock())
                            ->getEntry());
            }

            if (!((tempBlock->getExit()->getNextTreeTop())
                    && (tempBlock->getExit()->getNextTreeTop()->getNode()->getBlock()->isExtensionOfPreviousBlock())))
                break;
        ...
        for (auto edge = lastBlock->getSuccessors().begin(); edge != lastBlock->getSuccessors().end(); ++edge) {
            ...
            if (!(lastOpCode.isBranch() || lastOpCode.isJumpWithMultipleTargets() || lastOpCode.isReturn()
                    || (lastOpCode.getOpCodeValue() == TR::athrow)))
                fallsThrough = true;
            else if (lastOpCode.isBranch()) {
                if (!((lastNode->getBranchDestination() == succ->getEntry())
                        || (succ->getEntry()
                            && (lastNode->getBranchDestination()
                                == cloner.getToBlock(succ->getEntry()->getNode()->getBlock())->getEntry()))))
                    fallsThrough = true;
            }
            ...
            if (!fallsThrough) {
                ...
                logprintf(trace(), comp()->log(), "Creating new goto block : %d\n", newGotoBlock->getNumber());
        ...
        //
        for (auto edge = lastBlock->getExceptionSuccessors().begin(); edge != lastBlock->getExceptionSuccessors().end();
             ++edge) {
            TR::Block *succ = toBlock((*edge)->getTo());
        ...
        while (comparisonNode) {
            ...
            TR::Block *comparisonBlock
                = TR::Block::createEmptyBlock(insertionPoint->getNode(), comp(), block->getFrequency(), block);
            ...
            if (comp()->useCompressedPointers()) {
                for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
                    ...
                    if (objectRef->getOpCode().isLoadIndirect() && objectRef->getDataType() == TR::Address
                    ...
                    if (shouldBeCompressed) {
                        TR::TreeTop *translateTT
                            = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(objectRef), NULL, NULL);
...
bool OMR::ValuePropagation::prepareForBlockVersion(TR_LinkHead<ArrayLengthToVersion> *arrayLengths)
{
    ...
    for (TR::Node *bndchkNode = iter.getFirst(); bndchkNode; bndchkNode = iter.getNext()) {
        ...
        if (!arrayLen->getOpCode().isLoadDirect()) {
            ...
            if (arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload)
                symbolRef = arrayLen->getFirstChild()->getOpCode().hasSymbolReference()
                    ? arrayLen->getFirstChild()->getSymbolReference()
                    : 0;
            else
                symbolRef = arrayLen->getFirstChild()->getFirstChild()->getOpCode().hasSymbolReference()
                    ? arrayLen->getFirstChild()->getFirstChild()->getSymbolReference()
                    : 0;
            ...
            if ((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aloadi)
                && (arrayLen->getFirstChild()->getFirstChild()->hasUnresolvedSymbolReference()))
                continue;
            ...
            if (arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aloadi) {
                ...
                int32_t len;
                char *sig
                    = symRefField->getOwningMethod(comp())->classNameOfFieldOrStatic(symRefField->getCPIndex(), len);
        ...
        for (ArrayLengthToVersion *arrayFound = arrayLengths->getFirst(); arrayFound;
             arrayFound = arrayFound->getNext()) {
            ...
            if (index) {
                ...
                if (symRef) {
                    if (invariantVariables.isSet(symRef->getReferenceNumber()))
                        ...
                        for (FirstLoadOfNonInvariant *firstLoad = _firstLoads->getFirst(); firstLoad && !baseNode;
                             firstLoad = firstLoad->getNext()) {
                    ...
                    if (baseNode) // invarinat or trackableNonInvariant
                    {
                        ...
                        if (bndchkNode->getSecondChild() == baseNode)
                            ...
                            if (getConstraint(bndchkNode->getSecondChild(), isGlobal, baseNode))
                                c = getConstraint(bndchkNode->getSecondChild(), isGlobal, baseNode)
                                        ->asEqual()
                                        ->increment();
                            else
                                shouldAnalyze = false;
        ...
        for (ArrayIndexInfo *arrayIndexInfo = array->_arrayIndicesInfo->getFirst(); arrayIndexInfo;
             arrayIndexInfo = arrayIndexInfo->getNext()) {
            ...
            if (baseNode) {
                TR::VPConstraint *rel
                    = getConstraint(bndchkNode->getSecondChild(), isGlobal, arrayIndexInfo->_baseNode);
        ...
        if (arrayIndexInfoFound) {
            ...
            if (baseNode) {
                if (arrayIndexInfoFound->_max == arrayIndexInfoFound->_min) {
                    ...
                    } else if ((deltaMax < 0 && deltaMin > 0) || (deltaMax == 0 || deltaMin == 0))
                        removeBndchkFromFastBlock = true;
                }
            } else {
    ...
    for (ArrayLengthToVersion *array = arrayLengths->getFirst(); array; array = array->getNext()) {
        for (ArrayIndexInfo *arrayIndex = array->_arrayIndicesInfo->getFirst(); arrayIndex;
             arrayIndex = arrayIndex->getNext()) {
...
void OMR::ValuePropagation::buildBoundCheckComparisonNodes(BlockVersionInfo *blockInfo, List<TR::Node> *comparisonNodes)
{
    ...
    for (ArrayLengthToVersion *arrayLength = blockInfo->_arrayLengths->getFirst(); arrayLength;
         arrayLength = arrayLength->getNext()) {
            ...
            if (performTransformation(comp(), "%s Creating tests outside block_%d for versioning arraylenth %p \n",
                ...
                if (arrayIndex->_baseNode) {
                    ...
                    minIndex = TR::Node::create(TR::iadd, 2, arrayIndex->_baseNode->duplicateTree(),
                        TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0, arrayIndex->_min));
                } else {
                ...
                if (arrayIndex->_baseNode) {
                    nextComparisonNode = TR::Node::createif(TR::ificmplt, minIndex,
                        TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0, 0));

                    logprintf(trace(), comp()->log(), "First Test - Creating %p (%s)\n", nextComparisonNode,
                ...
                logprintf(trace(), comp()->log(), "Second test - Creating %p (%s)\n", nextComparisonNode,
                    nextComparisonNode->getOpCode().getName());
                ...
                if (arrayIndex->_baseNode && arrayIndex->_instanceOfClass
                    ...
                    if (indexObjectRefChild) {
                        ...
                        clazz = sig ? fe()->getClassFromSignature(sig, len,
                                          indexObjectRefChild->getSymbolReference()->getOwningMethod(comp()))
                                    : NULL;
                        TR_YesNoMaybe result
                            = clazz ? comp()->fe()->isInstanceOf(clazz, arrayIndex->_instanceOfClass, false) : TR_no;
                        if (result != TR_yes) {
                            if (comp()->compileRelocatableCode()) {
                                dumpOptDetails(comp(),
                                    "%s Abandoned versioning of block_%d because an instanceOf check is needed and "
                                    "this is a relocatable compile.\n",
                            ...
                            dumpOptDetails(comp(),
                                ...
                                "\n",
                                OPT_DETAILS, indexObjectRefChild, blockInfo->_block->getNumber(),
                                arrayIndex->_baseNode);
                            TR::Node *duplicateClassPtr
                                = TR::Node::createWithSymRef(indexObjectRefChild, TR::loadaddr, 0,
                            ...
                            TR::Node *instanceofNode = TR::Node::createWithSymRef(
                                TR:: instanceof, 2, 2, indexObjectRefChild->duplicateTree(), duplicateClassPtr,
                                comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
                            TR::Node *ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
                                TR::Node::create(arrayIndex->_baseNode, TR::iconst, 0, 0));
                            temp.add(ificmpeqNode);
                            numInstanceOfChecksAdded++;
                        } else {
                            dumpOptDetails(comp(),
                                "%s ctInstanceOf test passed, skipping runtime instanceof(%p, %p) of %p outside "
                                "block_%d for versioning array index %p\n",
                                OPT_DETAILS, clazz, arrayIndex->_instanceOfClass, indexObjectRefChild,
                                blockInfo->_block->getNumber(), arrayIndex->_baseNode);
                        }
                    }
                }
            }
        ...
        if (arrayLengthVersioned) {
            if (arrayLength->_arrayLen->getOpCode().isArrayLength()) {
                ...
                if (objectRefChild && !objectRefChild->isNonNull()) {
                    dumpOptDetails(comp(),
                        "%s Creating test for nullCheck of %p outside block_%d for versioning arraylenth %p \n",
                        OPT_DETAILS, objectRefChild, blockInfo->_block->getNumber(), arrayLength->_arrayLen);
                    TR::Node *nullTestNode = TR::Node::createif(TR::ifacmpeq, objectRefChild->duplicateTree(),
                        TR::Node::aconst(objectRefChild, 0));
                ...
                if (arrayLength->_instanceOfClass && objectRefChild) {
                    ...
                    clazz = sig ? fe()->getClassFromSignature(sig, len,
                                      objectRefChild->getSymbolReference()->getOwningMethod(comp()))
                                : NULL;
                    TR_YesNoMaybe result
                        = clazz ? comp()->fe()->isInstanceOf(clazz, arrayLength->_instanceOfClass, false) : TR_no;
                    ArrayIndexInfo *arrayIndex = NULL;

                    if (result == TR_yes) {
                        dumpOptDetails(comp(),
                            "%s ctInstanceOf test passed, skipping runtime instanceof(%p, %p) of %p outside block_%d "
                            "for versioning arraylenth %p\n",
                            OPT_DETAILS, clazz, arrayLength->_instanceOfClass, objectRefChild,
                            blockInfo->_block->getNumber(), arrayLength->_arrayLen);
                    } else {
                        // Have we already generated an instanceOf for this object/class when handling array indexes?
                        for (arrayIndex = arrayLength->_arrayIndicesInfo->getFirst(); result != TR_yes && arrayIndex;
                             arrayIndex = arrayIndex->getNext()) {
                            if (arrayIndex->_versionBucket && arrayIndex->_baseNode
                                && arrayIndex->_baseNode->getOpCode().getOpCodeValue() == TR::iloadi
                                && objectRefChild == arrayIndex->_baseNode->getFirstChild()
                                && arrayLength->_instanceOfClass == arrayIndex->_instanceOfClass) {
                                result = TR_yes; // We already generated an instanceOf check for this object&class
                                                 // combination
                                dumpOptDetails(comp(),
                                    "%s Skipping redundant runtime instanceof of %p outside block_%d for versioning "
                                    "arraylenth %p\n",
                                    OPT_DETAILS, objectRefChild, blockInfo->_block->getNumber(),
                                    arrayLength->_arrayLen);
                            }
                        }
                        ...
                        if (result != TR_yes) {
                            ...
                            dumpOptDetails(comp(),
                                "%s Creating test for instanceof of %p outside block_%d for versioning arraylenth %p "
                                "\n",
                                OPT_DETAILS, objectRefChild, blockInfo->_block->getNumber(), arrayLength->_arrayLen);
                            TR::Node *duplicateClassPtr = TR::Node::createWithSymRef(objectRefChild, TR::loadaddr, 0,
                                comp()->getSymRefTab()->findOrCreateClassSymbol(
                                    objectRef->getSymbolReference()->getOwningMethodSymbol(comp()), -1,
                                    arrayLength->_instanceOfClass, false));
                            TR::Node *instanceofNode = TR::Node::createWithSymRef(
                                TR:: instanceof, 2, 2, objectRefChild->duplicateTree(), duplicateClassPtr,
                                comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
                            TR::Node *ificmpeqNode = TR::Node::createif(TR::ificmpeq, instanceofNode,
                                TR::Node::create(objectRef, TR::iconst, 0, 0));
                ...
                if (!objectRef->isNonNull()) {
                    ...
                    TR::Node *nullTestNode
                        = TR::Node::createif(TR::ifacmpeq, objectRef->duplicateTree(), TR::Node::aconst(objectRef, 0));
                    comparisonNodes->add(nullTestNode);
                    numOfNullTestsAdded++;
                }
            }
    ...
    if (numBoundChecksRemoved <= 3 + (numInstanceOfChecksAdded * 3) + numOfNullTestsAdded) {
        dumpOptDetails(comp(),
            "%s Abandoned versioning of block_%d, it would require %d instanceOfChecks and %d null-tests while only "
            "saving %d boundChecks\n",
            OPT_DETAILS, blockInfo->_block->getNumber(), numInstanceOfChecksAdded, numOfNullTestsAdded,
            numBoundChecksRemoved);
        comparisonNodes->deleteAll();
    } else {
        dumpOptDetails(comp(),
            "%s Versioning of block_%d requires %d instanceOfChecks and %d null-tests while saving %d boundChecks\n",
            OPT_DETAILS, blockInfo->_block->getNumber(), numInstanceOfChecksAdded, numOfNullTestsAdded,
            numBoundChecksRemoved);
        ...
        for (ArrayIndexInfo *index = array->_arrayIndicesInfo->getFirst(); index; index = index->getNext()) {
            if (!index->_bndChecks->isEmpty() && !index->_bndChecks->isSingleton()) {
                ...
                for (TR::Node *bndchk = iter.getFirst(); bndchk; bndchk = iter.getNext()) {
                    ...
                    logprintf(trace(), comp()->log(), "Block versioner: Remove bndchk %p \n", bndchk);
...
TR::Node *OMR::ValuePropagation::findVarOfSimpleFormOld(
    TR::Node *node) // ArrayIndexNodeFromOffset(TR::Node *node, int32_t stride)
{
    if (node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference()
        && ((node->getOpCode().getOpCodeValue() == TR::iload)
            || ((node->getOpCode().getOpCodeValue() == TR::iloadi)
                && ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload)
                    ...
                    && !(_seenDefinedSymbolReferences->isSet(
        ...
        if (node && node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference()
            && ((node->getOpCode().getOpCodeValue() == TR::iload)
                || ((node->getOpCode().getOpCodeValue() == TR::iloadi)
                    && ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload)
                        && !((node->getFirstChild())->hasUnresolvedSymbolReference())
...
TR::Node *OMR::ValuePropagation::findVarOfSimpleForm(
    TR::Node *node) // ArrayIndexNodeFromOffset(TR::Node *node, int32_t stride)
{
    if (node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference()
        && ((node->getOpCode().getOpCodeValue() == TR::iload)
    ...
    if (opCode.hasSymbolReference()) {
        ...
        if (!symReference->getSymbol()->isAutoOrParm() || symReference->isUnresolved()) {
            if (node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference()
                && ((node->getOpCode().getOpCodeValue() == TR::iload)
                    || ((node->getOpCode().getOpCodeValue() == TR::iloadi)
                        && ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload)
            ...
            for (FirstLoadOfNonInvariant *firstLoad = _firstLoads->getFirst(); firstLoad;
                 firstLoad = firstLoad->getNext()) {
            ...
            if (!trackedInvariant && _seenDefinedSymbolReferences->get(symReference->getReferenceNumber()))
                return NULL;
        }
    }
...
void OMR::ValuePropagation::createNewBucketForArrayIndex(ArrayLengthToVersion *array,
    ...
    TR_OpaqueClassBlock *instanceOfClass, TR_OpaqueClassBlock *indexSourceObjectClass)
{
        ...
        if (!opCode.isLoadVar() && (opCode.getOpCodeValue() != TR::loadaddr)) {
            ...
            if (opCode.isStore()) {
                ...
                if (opCode.getOpCodeValue() == TR::istore) {
                    ...
                    if (load
                        && symReference->getReferenceNumber()
                            == load->getSymbolReference()->getReferenceNumber()) { // check if this is the first time
...
void OMR::ValuePropagation::addToSortedList(TR_LinkHead<ArrayLengthToVersion> *arrayLengths,
    ArrayLengthToVersion *newArrayLen)
{
...
TR::TreeTop *OMR::ValuePropagation::createReferenceArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
    TR::SymbolReference *srcObjectRef, TR::SymbolReference *dstObjectRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, bool useFlagsOnOriginalArraycopy)
{
    TR::Node *root = tree->getNode()->getFirstChild();
    TR::Node *len = TR::Node::createLoad(root, lenRef);
...
void OMR::ValuePropagation::transformReferenceArrayCopyWithoutCreatingStoreTrees(TR_TreeTopWrtBarFlag *arrayTree,
    TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef, TR::SymbolReference *srcRef,
    TR::SymbolReference *dstRef, TR::SymbolReference *lenRef)
{
    ...
    createReferenceArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyArrayStoreCheck, srcObjRef, dstObjRef, lenRef,
        srcRef, dstRef, true);
    ...
    if ((arrayTree->_flag & NEED_WRITE_BARRIER) == 0)
        createPrimitiveArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcRef, dstRef, lenRef,
            true, true);
    else {
        createReferenceArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcObjRef, dstObjRef,
            lenRef, srcRef, dstRef, true);
    ...
    arraycopyBlock->createConditionalBlocksBeforeTree(arrayTree->_treetop, ifTree, arraycopyArrayStoreCheck,
        arraycopyNoArrayStoreCheck, cfg, false);
...
void OMR::ValuePropagation::transformUnknownTypeArrayCopy(TR_TreeTopWrtBarFlag *arrayTree)
{
    ...
    TR_ASSERT(srcObjRef, "No source object ref\n");
    TR_ASSERT(dstObjRef, "No destination object ref\n");
    createReferenceArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyReference, srcObjRef, dstObjRef, lenRef, srcRef,
        dstRef, true);

    TR::Node *srcObject = node->getChild(0);
    TR::TreeTop *ifTree = createPrimitiveOrReferenceCompareNode(srcObject);

    arraycopyBlock->createConditionalBlocksBeforeTree(arrayTree->_treetop, ifTree, arraycopyReference,
        arraycopyPrimitive, cfg, false);
    ...
    if ((arrayTree->_flag & NEED_ARRAYSTORE_CHECK) == 0)
        ...
        transformReferenceArrayCopyWithoutCreatingStoreTrees(
            new (trStackMemory()) TR_TreeTopWrtBarFlag(arraycopyReference, arrayTree->_flag), srcObjRef, dstObjRef,
            srcRef, dstRef, lenRef);
#endif
}
...
bool OMR::ValuePropagation::isUnreliableSignatureType(TR_OpaqueClassBlock *klass, TR_OpaqueClassBlock *&erased)
{
    erased = klass;
    return false;
}

bool OMR::ValuePropagation::canArrayClassBeTrustedAsFixedClass(TR_OpaqueClassBlock *arrayClass,
...
void OMR::ValuePropagation::doDelayedTransformations()
{
    ...
    for (scalarizedArrayCopy = treesIt1.getFirst(); scalarizedArrayCopy; scalarizedArrayCopy = treesIt1.getNext()) {
        ...
        TR::TransformUtil::scalarizeArrayCopy(comp(), scalarizedArrayCopy->_node, scalarizedArrayCopy->_treetop, true,
            didTransformArrayCopy);
    ...
    if (comp()->requiresSpineChecks()) {
        ...
        for (arrayCopySpineCheck = acscIt.getFirst(); arrayCopySpineCheck; arrayCopySpineCheck = acscIt.getNext()) {
    ...
    for (converterCallTree = convIt.getFirst(); converterCallTree; converterCallTree = convIt.getNext()) {
    ...
    for (unknownTypeArrayCopyTree = treesIt.getFirst(); unknownTypeArrayCopyTree;
         unknownTypeArrayCopyTree = treesIt.getNext()) {
    ...
    for (referenceArrayCopyTree = treesIt.getFirst(); referenceArrayCopyTree;
         referenceArrayCopyTree = treesIt.getNext()) {
    ...
    if (TR::Compiler->om.areFlattenableValueTypesEnabled()) {
        ...
        for (nullRestrictedArrayCopyTree = tt.getFirst(); nullRestrictedArrayCopyTree;
    ...
    if (_edgesToBeRemoved) {
        for (i = _edgesToBeRemoved->size() - 1; i >= 0; --i) {
            ...
            if (std::find(edge->getTo()->getPredecessors().begin(), edge->getTo()->getPredecessors().end(), edge)
                ...
                logprintf(trace(), log, "Removing unreachable edge from %d to %d\n", edge->getFrom()->getNumber(),
                    edge->getTo()->getNumber());
                ...
                if (callNode->getOpCode().isCallIndirect()) {
                    ...
                    if (!guardForDifferentCall) {
                        ...
                        if (!guard) {
                            ...
                            if (foldedGuard && (foldedGuard != cfg->getStart())
                                && (foldedGuard->getSuccessors().size() == 1)
                                && (foldedGuard->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto)
                                && (foldedGuard->getLastRealTreeTop()->getNode()->getBranchDestination()
                                    == ci->_block->getEntry()))
                        ...
                        if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()) {
                            ...
                            if (guardNode) {
                                ...
                                canBeRemoved = virtualGuard->canBeRemoved()
                                    && callNode->getOpCode()
                                           .isCallDirect(); // do not remove guards for indirect calls. If the ifacmpne
                                                            // handler didn't do it, the chances are it is illegal to
                                                            // remove the guard
                            } else
                                canBeRemoved = true;

                            if (guardNode && guardNode->isProfiledGuard()) {
                                TR::Node *classTypeNode = guardNode->getSecondChild();

                                int32_t guardType = -1; // unknown
                                if (guardNode->getOpCodeValue() == TR::ifacmpne) {
                                    logprintf(trace(), log, "Got guard [%p] as ifacmpne\n", guardNode);
                                    guardType = 1; // change if to goto
                                } else if (guardNode->getOpCodeValue() == TR::ifacmpeq) {
                                    logprintf(trace(), log, "Got guard [%p] as ifacmpeq\n", guardNode);
                                ...
                                if (classTypeNode->getOpCodeValue() == TR::aconst
                                    ...
                                    TR_OpaqueClassBlock *receiverClass = callNode->getSymbolReference()
                                                                             ->getSymbol()
                                    ...
                                    if (ci->_thisType && (ci->_thisType != typeClass)) {
                                        isVPClassDerivedFromGuardClass
                                            = comp()->fe()->isInstanceOf(ci->_thisType, typeClass, true);
                                        if (isVPClassDerivedFromGuardClass == TR_no)
                                            isGuardClassDerivedFromVPClass
                                                = comp()->fe()->isInstanceOf(typeClass, ci->_thisType, true);
                                    }

                                    if ((result == TR_no) || (isVPClassDerivedFromGuardClass == TR_yes)
                                        || (isGuardClassDerivedFromVPClass == TR_no)) {
                                        typeCompatibleStatus = TR_no; // types incompatible
                                    } else if ((result == TR_yes) && (ci->_thisType == typeClass) && canBeRemoved) {
                                        typeCompatibleStatus = TR_yes; // types compatible
                                    } else
                                        callUnreachable = false;

                                    logprintf(trace(), log, "typeCompatibleStatus [%p] %s\n", guardNode,
                                        comp()->getDebug()->getName(typeCompatibleStatus));
                                } else {
                                    TR_ASSERT((classTypeNode->getOpCodeValue() == TR::aconst
                                                  && classTypeNode->isMethodPointerConstant()),
                                        "Devirtualization of guarded profiled calls only works currently for "
                                        "vft-guards or method compare guards");
                                    void *profiledMethod = (void *)classTypeNode->getAddress();
                                    void *devirtualizedMethod = callNode->getSymbolReference()
                                                                    ->getSymbol()
                                                                    ->castToResolvedMethodSymbol()
                                                                    ->getResolvedMethod()
                                ...
                                if (typeCompatibleStatus == TR_no) {
                                    ...
                                    if ((guardType == 1)
                                        && performTransformation(comp(),
                                            "%sChanging branch (guard) %p in block_%d to goto due to "
                                            "devirtualization\n",
                                        ...
                                        && performTransformation(comp(),
                                            "%sRemoving branch %p in block_%d due to devirtualization\n", OPT_DETAILS,
                                            guardNode, guard->getNumber())) {
                                        // remove branch
            ...
            if (!callGuarded || !callUnreachable) {
                ...
                if (methodSymbol->isJNI())
                    ...
                    if (comp()->getMethodHotness() <= warm && comp()->getOption(TR_DisableInliningDuringVPAtWarm)) {
                        ...
                        tryToInline = false;
                        logprintf(trace(), comp()->log(), "\tDo not inline call at [%p]: block frequency %d < %d\n",
    ...
    for (getComponentCallNode = nodesIt.getFirst(); getComponentCallNode; getComponentCallNode = nodesIt.getNext()) {
    ...
    for (predictedThrow = throwsIt.getFirst(); predictedThrow; predictedThrow = throwsIt.getNext()) {
        ...
        //
        while (firstTT && firstTT->getNode()->isProfilingCode())
        ...
        if (treeTop->getNode()->getOpCodeValue() == TR::NULLCHK) {
            ...
            treeTop = TR::TreeTop::create(comp(), treeTop,
                TR::Node::createWithSymRef(TR::astore, 1, 1, node->getFirstChild(), excpSymRef));
        } else {
        ...
        for (auto edge = predictedCatchBlock->getExceptionSuccessors().begin();
             edge != predictedCatchBlock->getExceptionSuccessors().end();)
            cfg->removeEdge(*(edge++));
    }
    ...
    for (TR_OpaqueClassBlock *prexClazz = prexCit.getCurrent(); prexClazz; prexClazz = prexCit.getNext()) {
        ...
        TR_PersistentClassInfo *classInfo
            = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(prexClazz, comp());
        if (classInfo) {
            ...
            for (TR_PersistentClassInfo *subClassInfo = subClassesIt.getFirst(); subClassInfo;
                 subClassInfo = subClassesIt.getNext()) {
    ...
    for (ClassInitInfo *cii = _classesToCheckInit.getFirst(); cii; cii = cii->getNext()) {
        ...
        if ((origFirst->getOpCodeValue() == TR::aload) && origFirst->getSymbol()->isStaticField()) {
            int32_t staticNameLen = -1;
            char *staticName = NULL;
            staticName = origFirst->getSymbolReference()->getOwningMethod(comp())->staticName(
                origFirst->getSymbolReference()->getCPIndex(), staticNameLen, comp()->trMemory());
        ...
        if (!recognizedStatic) {
            ...
            TR::SymbolReference *newSymbolReference
                = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType, false, 0);

            TR::Node *astoreNode
                = TR::Node::createWithSymRef(TR::astore, 1, 1, ifNode->getFirstChild(), newSymbolReference);
        ...
        bool found = false;
        for (TR_ClassLoadCheck *clc = comp()->getClassesThatShouldNotBeLoaded()->getFirst(); clc;
             clc = clc->getNext()) {
        ...
        if (!found)
            comp()->getClassesThatShouldNotBeLoaded()->add(
                new (trHeapMemory()) TR_ClassLoadCheck(cii->_sig, cii->_len));
    }

    _classesToCheckInit.setFirst(0);
}

TR_OpaqueClassBlock *OMR::ValuePropagation::findLikelySubtype(TR_OpaqueClassBlock *klass) { return NULL; }

TR_OpaqueClassBlock *OMR::ValuePropagation::findLikelySubtype(const char *sig, int32_t len,
    TR_ResolvedMethod *owningMethod)
{
    return NULL;
}

TR::VPConstraint *OMR::ValuePropagation::createTypeHintConstraint(TR_ResolvedMethod *owningMethod, const char *sig,
    int32_t len)
{
    return NULL;
}

void constrainRangeByPrecision(const int64_t low, const int64_t high, const int32_t precision, int64_t &lowResult,
    int64_t &highResult, bool isNonNegative)
{
    lowResult = low;
    highResult = high;
...
void OMR::ValuePropagation::ValueConstraintHandler::setVP(OMR::ValuePropagation *vp) { _vp = vp; }
...
void OMR::ValuePropagation::ValueConstraintHandler::free(ValueConstraint *vc) { _vp->freeValueConstraint(vc); }
...
TR::Compilation *OMR::ValuePropagation::ValueConstraintHandler::comp() { return _vp->comp(); }

OMR::ValuePropagation::InductionVariable::InductionVariable(TR::Symbol *sym, TR::Node *entryDef, int32_t incrVN,
    TR::VPConstraint *incr, OMR::ValuePropagation *vp)
    : _symbol(sym)
    , _entryDef(entryDef)
    , _entryConstraint(0)
    , _incrementVN(incrVN)
...
OMR::ValuePropagation::CallInfo::CallInfo(OMR::ValuePropagation *vp, TR_OpaqueClassBlock *thisType,
...
OMR::ValuePropagation::VirtualGuardInfo::VirtualGuardInfo(OMR::ValuePropagation *vp, TR_VirtualGuard *vgOld,
    TR_VirtualGuard *vgNew, TR::Node *newGNode, TR::Node *cn)
    : _currentTree(vp->_curTree)
    , _block(vp->_curBlock)
    , _oldVirtualGuard(vgOld)
    , _newVirtualGuard(vgNew)
    , _newGuardNode(newGNode)
    , _callNode(cn)
{}
