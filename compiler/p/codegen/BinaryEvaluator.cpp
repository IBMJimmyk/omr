...
static inline TR::Register *iorTypeEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic immedOp,
    TR::InstOpCode::Mnemonic immedShiftedOp, TR::InstOpCode::Mnemonic regOp, TR::InstOpCode::Mnemonic regOp_r,
    TR::CodeGenerator *cg)
{
...
static bool genNullTestForCompressedPointers(TR::Node *node, TR::Register *&trgReg, TR::Register *&condReg,
    TR::Register *src1Reg, TR::Register *&src2Reg, TR::LabelSymbol *&doneSkipAdd, TR::CodeGenerator *cg)
{
    if (cg->comp()->useCompressedPointers() && node->containsCompressionSequence()) {
        TR::Node *n = node;
        ...
        if (n->getOpCodeValue() == TR::ladd || n->getOpCodeValue() == TR::lsub) {
            ...
            if (n->getFirstChild()->getOpCodeValue() == TR::iu2l || n->getFirstChild()->getOpCodeValue() == TR::a2l
        ...
        if (!node->getSecondChild()->getOpCode().isLoadConst() || node->getSecondChild()->getRegister())
...
TR::Register *OMR::Power::TreeEvaluator::iaddEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P9) && firstChild->getOpCodeValue() == TR::imul
...
static void genericLongAnalyzer(TR::CodeGenerator *cg, TR::Node *&child, TR::Register *&lowReg, TR::Register *&highReg,
    bool &highZero)
{
    ...
    if (child->isHighWordZero()) {
        ...
        if (child->getReferenceCount() == 1 && reg == NULL) {
            if (firstOp == TR::iu2l || firstOp == TR::su2l
                || (firstOp == TR::lushr && (child->getSecondChild()->getOpCodeValue() == TR::iconst)
...
static TR::Register *carrylessLongEvaluatorWithAnalyser(TR::Node *node, TR::CodeGenerator *cg,
    TR::InstOpCode::Mnemonic lowRegRegOpCode, TR::InstOpCode::Mnemonic highRegRegOpCode,
        ...
        if (!setsOrReadsCC && secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (!setsOrReadsCC && (firstChild->isHighWordZero() || secondChild->isHighWordZero())) {
                ...
                if ((node->getOpCodeValue() == TR::luaddc)
                    && TR_PPCComputeCC::setCarryBorrow(node->getChild(2), false, &carryReg, cg)) {
                    // use adde rather than addc
                    regToRegOpCode = TR::InstOpCode::adde;
                }
                generateTrg1Src2Instruction(cg, regToRegOpCode, node, lowReg, src1Reg->getLowOrder(),
                    src2Reg->getLowOrder());
                generateTrg1Src2Instruction(cg, TR::InstOpCode::adde, node, highReg, src1Reg->getHighOrder(),
                    src2Reg->getHighOrder());
            }
        }
    } else // 64 bit target
    {
        ...
        hasCompressedPointers
            = genNullTestForCompressedPointers(node, trgReg, condReg, src1Reg, src2Reg, doneSkipAdd, cg);
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P9) && !setsOrReadsCC
            && (node->getOpCodeValue() == TR::ladd || node->getOpCodeValue() == TR::aladd)
            && firstChild->getOpCodeValue() == TR::lmul && firstChild->getReferenceCount() == 1
            ...
            // to be an lconst
            else if (!setsOrReadsCC && (secondOp == TR::iconst) && // may be true if aladd?
...
TR::Register *lsub64Evaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->useCompressedPointers() && node->containsCompressionSequence()) {
        ...
        if (!setsOrReadsCC && (secondChild->getOpCode().isLoadConst()) && secondChild->getRegister() == NULL) {
            src1Reg = cg->evaluate(firstChild);
            ...
            if (!setsOrReadsCC && (firstChild->getOpCode().isLoadConst()) && firstChild->getRegister() == NULL) {
                trgReg = cg->allocateRegister();
            ...
            } else // no constants or setsOrReadsCC
            {
                ...
                if (setsOrReadsCC) {
                    ...
                    if ((node->getOpCodeValue() == TR::lusubb)
                        && TR_PPCComputeCC::setCarryBorrow(node->getChild(2), true, &borrowReg, cg)) {
...
TR::Register *OMR::Power::TreeEvaluator::lsubEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!setsOrReadsCC && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        TR::Register *src1Reg = cg->evaluate(firstChild);
        int64_t longValue = secondChild->getLongInt();
        longValue = -longValue;
        int32_t lowValue = (int32_t)longValue;
        int32_t highValue = (int32_t)(longValue >> 32);
        trgReg = addConstantToLong(node, src1Reg->getHighOrder(), src1Reg->getLowOrder(), highValue, lowValue, cg);

    } else {
        TR::Register *lowReg = cg->allocateRegister();
        TR::Register *highReg = cg->allocateRegister();
        if (!setsOrReadsCC && firstChild->getOpCodeValue() == TR::lconst && firstChild->getRegister() == NULL) {
            TR::Register *src2Reg = cg->evaluate(secondChild);
            ...
            if (lowValue >= LOWER_IMMED && lowValue <= UPPER_IMMED) {
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::subfic, node, lowReg, src2Reg->getLowOrder(),
                    lowValue);
            } else {
                TR::Register *tempReg = cg->allocateRegister();
            ...
            generateTrg1Src2Instruction(cg, regToRegOpCode, node, lowReg, src2Reg->getLowOrder(),
                src1Reg->getLowOrder());
            generateTrg1Src2Instruction(cg, TR::InstOpCode::subfe, node, highReg, src2Reg->getHighOrder(),
                src1Reg->getHighOrder());
        }
        trgReg = cg->allocateRegisterPair(lowReg, highReg);
    }
...
//
TR::Register *OMR::Power::TreeEvaluator::dualMulHelper64(TR::Node *node, TR::Node *lmulNode, TR::Node *lumulhNode,
    TR::CodeGenerator *cg)
{
    ...
    if (!needsUnsignedHighMulOnly) {
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::mulld, node, lmulTargetRegister, lumulhTargetRegister,
            secondRegister);
        lmulNode->setRegister(lmulTargetRegister);
    }
    generateTrg1Src2Instruction(cg, TR::InstOpCode::mulhdu, node, lumulhTargetRegister, lumulhTargetRegister,
        secondRegister);
...
//
TR::Register *OMR::Power::TreeEvaluator::dualMulHelper32(TR::Node *node, TR::Node *lmulNode, TR::Node *lumulhNode,
    TR::CodeGenerator *cg)
{
    bool needsUnsignedHighMulOnly = (lmulNode == NULL);
    // requires:
...
//
TR::Register *OMR::Power::TreeEvaluator::dualMulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR_ASSERT(node->isDualCyclic() || needsUnsignedHighMulOnly,
        "Should be either calculating cyclic dual or just the high part of the lmul.");
...
TR::Register *OMR::Power::TreeEvaluator::lmulEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if (!(lowValue == 0 && highValue == 0) && highValue >= 0 && cg->convertMultiplyToShift(node)) {
            // The multiply has been converted to a shift.
            // Note that we have restricted this to positive constant multipliers.
            // We can do it for negative ones too, but then the result of the shift
            // operation needs to be negated here after the shift node is evaluated.
...
static TR::Register *signedIntegerDivisionOrRemainderAnalyser(TR::Node *node, TR::CodeGenerator *cg,
    TR::Register *dividendReg, int32_t divisorValue, bool isRemainder, TR::Register *trgReg = NULL,
    TR::Register *divisorReg = NULL, TR::Register *tmp1Reg = NULL, TR::Register *tmp2Reg = NULL)
{
    ...
    if (divisorValue == 1) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, trgReg, dividendReg,
            trailingZeroes(divisorValue));
...
static TR::Register *signedLongDivisionOrRemainderAnalyser(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (isPowerOf2(divisor)) {
        ...
        if (rootOpCode == TR::lrem) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::sradi, node, temp1Reg, dividendReg,
                trailingZeroes(divisor));
...
TR::Register *OMR::Power::TreeEvaluator::idivEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (divisor != 0) {
        trgReg = signedIntegerDivisionOrRemainderAnalyser(node, cg, dividendReg, divisor, false, NULL,
        ...
        if (testNeeded) {
            TR::PPCControlFlowInstruction *cfop
                = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(cg, TR::InstOpCode::idiv, node);
            condReg = cg->allocateRegister(TR_CCR);
...
static TR::Register *ldiv64Evaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (divisor != CONSTANT64(0)) {
        ...
        if (testNeeded) {
            TR::PPCControlFlowInstruction *cfop
                = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(cg, TR::InstOpCode::ldiv, node);
            condReg = cg->allocateRegister(TR_CCR);
            cfop->addTargetRegister(condReg);
            cfop->addTargetRegister(trgReg);
            cfop->addSourceRegister(dividendReg);
...
static void strengthReducingLongDivideOrRemainder32BitMode(TR::Node *node, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *dependencies, TR::Register **dd_highReg, TR::Register **dd_lowReg,
    TR::Register **dr_highReg, TR::Register **dr_lowReg, bool isSignedOp, bool isRemainder)
{
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    ...
    *dr_lowReg = dr_l;
    ...
    bool isDividendImpossible32Bit = firstChild->getOpCode().isLoadConst()
        && (ddConst > (int64_t)TR::getMaxSigned<TR::Int32>() || ddConst < (int64_t)TR::getMinSigned<TR::Int32>());
    bool isDivisorImpossible32Bit = secondChild->getOpCode().isLoadConst()
        && (drConst > (int64_t)TR::getMaxSigned<TR::Int32>() || drConst < (int64_t)TR::getMinSigned<TR::Int32>());
    if (!isDividendImpossible32Bit && !isDivisorImpossible32Bit) {
        TR::LabelSymbol *callLabel = generateLabelSymbol(cg);

        if (secondChild->getOpCode().isLoadConst() && drConst <= (int64_t)TR::getMaxSigned<TR::Int32>()
            && drConst >= (int64_t)TR::getMinSigned<TR::Int32>() && drConst != -1 && drConst != 0) {
            ...
            signedIntegerDivisionOrRemainderAnalyser(node, cg, dd_l, drConst, isRemainder, isRemainder ? dr_l : dd_l,
                dr_l, tmp1Reg, tmp2Reg);
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, isRemainder ? dr_h : dd_h,
                isRemainder ? dr_l : dd_l, 31);
        } else {
...
TR::Register *OMR::Power::TreeEvaluator::ldivEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    TR::RegisterDependencyConditions *dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(14, 14, cg->trMemory());
    bool signed_div = (node->getOpCodeValue() == TR::ldiv);

    strengthReducingLongDivideOrRemainder32BitMode(node, cg, dependencies, &dd_highReg, &dd_lowReg, &dr_highReg,
        &dr_lowReg, signed_div, false);
...
TR::Register *OMR::Power::TreeEvaluator::iremEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (divisor != 0) {
        trgReg = signedIntegerDivisionOrRemainderAnalyser(node, cg, dividendReg, divisor, true, NULL,
            secondChild->getRegister());
    } else {
        bool testNeeded = (!(secondChild->isNonNegative()) && !(firstChild->isNonNegative()));
        TR::Register *condReg;
        ...
        if (testNeeded) {
            TR::PPCControlFlowInstruction *cfop
                = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(cg, TR::InstOpCode::irem, node);
            condReg = cg->allocateRegister(TR_CCR);
            cfop->addTargetRegister(condReg);
            cfop->addTargetRegister(trgReg);
            cfop->addSourceRegister(dividendReg);
...
TR::Register *lrem64Evaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (divisor != 0) {
        ...
        if (testNeeded) {
            TR::PPCControlFlowInstruction *cfop
                = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(cg, TR::InstOpCode::lrem, node);
            condReg = cg->allocateRegister(TR_CCR);
            cfop->addTargetRegister(condReg);
            cfop->addTargetRegister(trgReg);
            cfop->addSourceRegister(dividendReg);
...
static bool isZeroExtendThenShiftLeftCandidate(TR::CodeGenerator *cg, TR::Node *node)
{
    ...
    if (!(lhs->getOpCode().isZeroExtension()
            || (lhs->getOpCode().isSignExtension() && lhs->getFirstChild()->isNonNegative())))
        return false;

    return performTransformation(cg->comp(),
        "O^O MASK/SHIFT MERGING: combining shift n%dn and zero-extending conversion n%dn into one operation\n",
        node->getGlobalIndex(), lhs->getGlobalIndex());
}
...
static bool isMaskThenShiftRightCandidate(TR::CodeGenerator *cg, TR::Node *node, uint32_t operandBits,
    uint64_t shiftAmount, bool isUnsigned)
{
    static bool disableMaskAndShiftMerging = feGetEnv("TR_DisableMaskAndShiftMerging");
    if (disableMaskAndShiftMerging)
        return false;
    ...
    return performTransformation(cg->comp(),
        "O^O MASK/SHIFT MERGING: combining shift n%dn and mask n%dn into one operation\n", node->getGlobalIndex(),
        lhs->getGlobalIndex());
}
...
static TR::Register *integerShiftRight(TR::Node *node, uint32_t operandSize, bool isUnsigned, TR::CodeGenerator *cg)
{
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (isMaskThenShiftRightCandidate(cg, node, operandBits, rhs, isUnsigned)) {
            ...
            TR::Register *lhs = cg->evaluate(lhsNode);
            uint64_t mask
                = (operandMask & node->getFirstChild()->getSecondChild()->get64bitIntegralValueAsUnsigned()) >> rhs;
...
static TR::Register *lshr32Evaluator(TR::Node *node, bool isUnsigned, TR::CodeGenerator *cg)
{
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (lhsNode->getOpCode().isLoadConst() && lhsNode->getLongIntHigh() == 0) {
            ...
            cg->stopUsingRegister(lhsLow);
        } else if (lhsNode->getOpCode().isLoadConst() && lhsNode->getLongIntHigh() == -1
            && lhsNode->getLongIntLow() < 0) {
            TR::Register *lhsLow = cg->allocateRegister();
            loadConstant(cg, node, lhsNode->getLongIntLow(), lhsLow);
...
TR::Register *OMR::Power::TreeEvaluator::irolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, targetRegister, srcRegister, shiftAmount,
            0xffffffff);
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::rlwnm, node, targetRegister, srcRegister, shiftAmountReg,
            0xffffffff);
    }
...
TR::Register *OMR::Power::TreeEvaluator::lrolEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, targetRegister, srcRegister, shiftAmount,
            CONSTANT64(0xffffffffffffffff));
    } else {
        TR::Register *shiftAmountReg = cg->evaluate(secondChild);
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::rldcl, node, targetRegister, srcRegister, shiftAmountReg,
            CONSTANT64(0xffffffffffffffff));
    }
...
void simplifyANDRegImm(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int64_t value, TR::CodeGenerator *cg,
...
void simplifyANDRegImm(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t value, TR::CodeGenerator *cg,
    TR::Node *constNode)
{
    ...
    if (localVal.getValue() == 0) {
        ...
        TR::Register *tmpReg = cg->allocateRegister(TR_CCR);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, node, trgReg, srcReg, tmpReg,
            localVal.getHighBits());
        cg->stopUsingRegister(tmpReg);
    } else {
        TR::Register *tmpReg;
        if (constNode != NULL) {
...
// We want to ensure that constants don't get reloaded into registers multiple times, and also to ensure that constants
// are not loaded into registers when an immediate form of an instruction will do.  This method is used for 'long and'
// on a 32 bit platform.  We will call the normal simplifyANDRegImm for a 32 bit value for each of the high and low
// halves on as long as we don't expect both to load their respective constants into a register. In that case we
// evaluate the constant node so it can be commoned.
void simplifyANDRegImm(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, TR::Node *constNode,
    TR::CodeGenerator *cg)
{
    int32_t lowValue;
    int32_t highValue;
    ...
    if ((localLow.getValue() == -1) || (contiguousBits(localLow.getValue())) || (localLow.getHighBits() == 0)
        || (localLow.getLowBits() == 0)) {
        lowSpecial = true;
    }

    if ((localHigh.getValue() == -1) || (contiguousBits(localHigh.getValue())) || (localHigh.getHighBits() == 0)
        || (localHigh.getLowBits() == 0)) {
        highSpecial = true;
    }
    ...
    if (lowSpecial == true || highSpecial == true) {
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg->getLowOrder(), srcReg->getLowOrder(),
            constReg->getLowOrder());
        generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg->getHighOrder(), srcReg->getHighOrder(),
            constReg->getHighOrder());
    }
}
...
TR::Register *OMR::Power::TreeEvaluator::landEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (secondOp == TR::lconst && secondChild->getRegister() == NULL) {
    ...
    } else // 32 bit target
    {
        if (secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (firstChild->isHighWordZero() || secondChild->isHighWordZero()) {
                return carrylessLongEvaluatorWithAnalyser(node, cg, TR::InstOpCode::AND, TR::InstOpCode::AND,
                    TR::InstOpCode::mr);
            } else {
                TR::Register *src1Reg = cg->evaluate(firstChild);
                TR::Register *src2Reg = cg->evaluate(secondChild);
                ...
                trgReg = cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
                generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg->getLowOrder(),
                    src2Reg->getLowOrder(), src1Reg->getLowOrder());
                generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg->getHighOrder(),
                    src2Reg->getHighOrder(), src1Reg->getHighOrder());
            }
        }
    }
    node->setRegister(trgReg);
...
static inline TR::Register *lorTypeEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic immedOp,
    TR::InstOpCode::Mnemonic immedShiftedOp, TR::InstOpCode::Mnemonic regOp, TR::InstOpCode::Mnemonic regOp_r,
    TR::CodeGenerator *cg)
{
    TR::Register *trgReg = NULL;
    ...
    } else // 32 bit target
    {
        ...
        if (secondOp == TR::lconst && secondChild->getRegister() == NULL) {
            ...
            if (localVal.getValue() == 0) {
                generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, trgReg->getLowOrder(),
                    src1Reg->getLowOrder());
                ...
                if (localVal.getHighBits() == 0) {
                    generateTrg1Src1ImmInstruction(cg, immedOp, node, trgReg->getLowOrder(), src1Reg->getLowOrder(),
                        localVal.getLowBits());
                } else if (localVal.getLowBits() == 0) {
                    generateTrg1Src1ImmInstruction(cg, immedShiftedOp, node, trgReg->getLowOrder(),
                        src1Reg->getLowOrder(), localVal.getHighBits());
                } else {
                    TR::Register *tempReg = cg->allocateRegister();
                    generateTrg1Src1ImmInstruction(cg, immedOp, node, tempReg, src1Reg->getLowOrder(),
                        localVal.getLowBits());
                    generateTrg1Src1ImmInstruction(cg, immedShiftedOp, node, trgReg->getLowOrder(), tempReg,
                        localVal.getHighBits());
                    cg->stopUsingRegister(tempReg);
                }
            }
            ...
            if (localVal.getValue() == 0) {
                generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, trgReg->getHighOrder(),
                    src1Reg->getHighOrder());
            } else if (localVal.getValue() == -1 && (node->getOpCodeValue() == TR::lor)) {
                generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg->getHighOrder(), -1);
            } else {
                if (localVal.getHighBits() == 0) {
                    generateTrg1Src1ImmInstruction(cg, immedOp, node, trgReg->getHighOrder(), src1Reg->getHighOrder(),
                        localVal.getLowBits());
                } else if (localVal.getLowBits() == 0) {
                    generateTrg1Src1ImmInstruction(cg, immedShiftedOp, node, trgReg->getHighOrder(),
                        src1Reg->getHighOrder(), localVal.getHighBits());
                } else {
                    TR::Register *tempReg = cg->allocateRegister();
                    generateTrg1Src1ImmInstruction(cg, immedOp, node, tempReg, src1Reg->getHighOrder(),
                        localVal.getLowBits());
                    generateTrg1Src1ImmInstruction(cg, immedShiftedOp, node, trgReg->getHighOrder(), tempReg,
                        localVal.getHighBits());
                    cg->stopUsingRegister(tempReg);
                }
            }
            ...
            TR::Register *src2Reg = cg->evaluate(secondChild);
            generateTrg1Src2Instruction(cg, regOp, node, trgReg->getLowOrder(), src2Reg->getLowOrder(),
                src1Reg->getLowOrder());
            generateTrg1Src2Instruction(cg, regOp, node, trgReg->getHighOrder(), src2Reg->getHighOrder(),
                src1Reg->getHighOrder());
        }
    }
    node->setRegister(trgReg);
...
TR::Register *OMR::Power::TreeEvaluator::lorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    {
        return carrylessLongEvaluatorWithAnalyser(node, cg, TR::InstOpCode::OR, TR::InstOpCode::OR, TR::InstOpCode::mr);
    } else {
        return lorTypeEvaluator(node, TR::InstOpCode::ori, TR::InstOpCode::oris, TR::InstOpCode::OR,
            TR::InstOpCode::or_r, cg);
    }
}
...
TR::Register *OMR::Power::TreeEvaluator::lxorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if ((node->getFirstChild()->isHighWordZero() || node->getSecondChild()->isHighWordZero())
        && !(secondOp == TR::lconst && node->getSecondChild()->getRegister() == NULL)
        && !(cg->comp()->target().is64Bit())) {
        return carrylessLongEvaluatorWithAnalyser(node, cg, TR::InstOpCode::XOR, TR::InstOpCode::XOR,
            TR::InstOpCode::mr);
    } else {
        return lorTypeEvaluator(node, TR::InstOpCode::xori, TR::InstOpCode::xoris, TR::InstOpCode::XOR,
            TR::InstOpCode::xor_r, cg);
    }
}
        ...
        if (cg->isRotateAndMask(node)) {
            if (firstChild->getOpCodeValue() == TR::imul) {
                ...
                if ((secondChild->getInt() & (0xffffffff << shiftAmount)) == 0)
                    ...
                    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, trgReg,
                        cg->evaluate(firstChild->getFirstChild()), shiftAmount,
                        secondChild->getInt() & (0xffffffff << shiftAmount));
            } else // ishr or iushr
            {
                int32_t shiftAmount = firstChild->getSecondChild()->getInt();
                generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, trgReg,
                    cg->evaluate(firstChild->getFirstChild()), 32 - shiftAmount,
                    secondChild->getInt() & (((uint32_t)0xffffffff) >> shiftAmount));
            }
            cg->decReferenceCount(firstChild->getFirstChild());
            cg->decReferenceCount(firstChild->getSecondChild());
        } else
            simplifyANDRegImm(node, trgReg, cg->evaluate(firstChild), (int32_t)secondChild->get64bitIntegralValue(), cg,
                secondChild);
    } else {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, trgReg, cg->evaluate(firstChild),
...
{
    return iorTypeEvaluator(node, TR::InstOpCode::ori, TR::InstOpCode::oris, TR::InstOpCode::OR, TR::InstOpCode::or_r,
        cg);
}

TR::Register *OMR::Power::TreeEvaluator::ixorEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    return iorTypeEvaluator(node, TR::InstOpCode::xori, TR::InstOpCode::xoris, TR::InstOpCode::XOR,
        TR::InstOpCode::xor_r, cg);
}
