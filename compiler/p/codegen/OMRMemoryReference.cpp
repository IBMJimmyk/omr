...
TR::MemoryReference *TR::MemoryReference::createWithLabel(TR::CodeGenerator *cg, TR::LabelSymbol *label, int64_t offset,
    int8_t length)
{
...
TR::MemoryReference *TR::MemoryReference::createWithIndexReg(TR::CodeGenerator *cg, TR::Register *baseReg,
    TR::Register *indexReg, uint8_t length)
{
...
TR::MemoryReference *TR::MemoryReference::createWithSymRef(TR::CodeGenerator *cg, TR::Node *node,
    TR::SymbolReference *symRef, uint32_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(node, symRef, length, cg);
}

TR::MemoryReference *TR::MemoryReference::createWithMemRef(TR::CodeGenerator *cg, TR::Node *node,
    TR::MemoryReference &memRef, int32_t displacement, uint32_t length)
{
    return new (cg->trHeapMemory()) TR::MemoryReference(node, memRef, displacement, length, cg);
}

OMR::Power::MemoryReference::MemoryReference(TR::CodeGenerator *cg)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
...
OMR::Power::MemoryReference::MemoryReference(TR::LabelSymbol *label, int64_t disp, uint8_t len, TR::CodeGenerator *cg)
    : _baseRegister(NULL)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
...
OMR::Power::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir, uint8_t len, TR::CodeGenerator *cg)
    : _baseRegister(br)
    , _baseNode(NULL)
    , _indexRegister(ir)
    , _indexNode(NULL)
...
OMR::Power::MemoryReference::MemoryReference(TR::Register *br, int64_t disp, uint8_t len, TR::CodeGenerator *cg)
    : _baseRegister(br)
    , _baseNode(NULL)
    , _indexRegister(NULL)
    , _indexNode(NULL)
        ...
        if (!ref->isUnresolved() && isLocalObject) {
            ...
            if (ref->isUnresolved()) {
                ...
                self()->setUnresolvedSnippet(
                    new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, rootLoadOrStore, ref, isStore, false));
...
OMR::Power::MemoryReference::MemoryReference(TR::Node *node, TR::MemoryReference &mr, int32_t displacement,
    uint32_t len, TR::CodeGenerator *cg)
{
    ...
    if (mr.getUnresolvedSnippet() != NULL) {
        _unresolvedSnippet = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, node, _symbolReference,
            mr.getUnresolvedSnippet()->isUnresolvedStore(), false);
...
void OMR::Power::MemoryReference::addToOffset(TR::Node *node, int64_t amount, TR::CodeGenerator *cg)
{
    ...
    //
    TR::Compilation *comp = cg->comp();
    if (self()->getUnresolvedSnippet() != NULL && (amount > LOWER_IMMED && amount < UPPER_IMMED)) {
    ...
    if (((displacement < LOWER_IMMED || displacement > UPPER_IMMED)
            && !cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10))
        ...
        if (_baseRegister == newBase && _baseNode == NULL)
            _baseRegister = NULL;
...
static bool isLoadConstAndShift(TR::Node *subTree, TR::CodeGenerator *cg)
{
    ...
    {
        if (subTree->getOpCodeValue() == TR::ishl && subTree->getFirstChild()->getOpCode().isLoadConst()
            && subTree->getSecondChild()->getOpCode().isLoadConst())
            return true;
        else
            return false;
...
void OMR::Power::MemoryReference::populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg)
{
    ...
    // Skip sign extension if the offset is known to be >= 0; this is only safe to do if the child is the result of a
    // load, otherwise the upper 32 bits may be undefined
    if (subTree->getOpCodeValue() == TR::i2l && subTree->isNonNegative()
        && subTree->getFirstChild()->getOpCode().isLoadVar() && subTree->getReferenceCount() == 1
        && subTree->getRegister() == NULL) {
    ...
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCode().isArrayRef()
            ...
            if (integerChild->getOpCode().isLoadConst()) {
                ...
                if (cg->comp()->target().is64Bit()) {
                    int64_t amount = (integerChild->getOpCodeValue() == TR::iconst) ? integerChild->getInt()
    ...
    TR::CodeGenerator *cg)
{
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
        ...
        if (_indexRegister != NULL) {
            ...
            generateTrg1Src2Instruction(cg, TR::InstOpCode::add, tempNode, tempTargetRegister, _baseRegister,
                _indexRegister);
...
void OMR::Power::MemoryReference::assignRegisters(TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
{
    ...
    if (_baseRegister != NULL) {
        ...
        if ((assignedBaseRegister != NULL)
            && (toRealRegister(assignedBaseRegister)->getRegisterNumber() == TR::RealRegister::gr0)) {
            TR::RealRegister *alternativeRegister;
            if ((alternativeRegister
                    = machine->findBestFreeRegister(currentInstruction, TR_GPR, true, false, _baseRegister))
                == NULL) {
                alternativeRegister = machine->freeBestRegister(currentInstruction, _baseRegister, NULL, true);
            }
            machine->coerceRegisterAssignment(currentInstruction, _baseRegister,
                toRealRegister(alternativeRegister)->getRegisterNumber());
...
void OMR::Power::MemoryReference::mapOpCode(TR::Instruction *currentInstruction)
{
    ...
    } else if ((self()->getUnresolvedSnippet() == NULL)
        && (self()->getOffset() < LOWER_IMMED || self()->getOffset() > UPPER_IMMED || self()->getLabel())
        && currentInstruction->cg()->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
        switch (currentInstruction->getOpCodeValue()) {
...
TR::Instruction *OMR::Power::MemoryReference::expandForUnresolvedSnippet(TR::Instruction *currentInstruction,
    TR::CodeGenerator *cg)
{
    ...
    TR::Instruction *firstInstruction = generateLabelInstruction(cg, TR::InstOpCode::bl, node,
        getUnresolvedSnippet()->getSnippetLabel(), prevInstruction);
    ...
    if (comp->target().is64Bit() && self()->isTOCAccess()) {
        ...
        if (displacement == PTOC_FULL_INDEX) {
            prevInstruction
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, data, data, 0, prevInstruction);
            prevInstruction = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, data, data, 32,
                0xffffffff00000000ULL, prevInstruction);
            prevInstruction
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::oris, node, data, data, 0, prevInstruction);
            prevInstruction
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, data, data, 0, prevInstruction);
        } else if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
            newMR = TR::MemoryReference::createWithDisplacement(cg, self()->getModBase(), LO_VALUE(displacement),
                self()->getLength());
        }
    } else if (index != NULL || isUsingDelayedIndexedForm()) {
        if (index == NULL) {
            newMR = TR::MemoryReference::createWithIndexReg(cg, cg->machine()->getRealRegister(TR::RealRegister::gr0),
                base, self()->getLength());
        } else {
        ...
        prevInstruction
            = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, base, base, 0, prevInstruction);
    } else {
    ...
    if (newMR != NULL) {
        switch (currentInstruction->getKind()) {
            case TR::Instruction::IsMem:
                prevInstruction = generateMemInstruction(cg, currentInstruction->getOpCodeValue(),
                    currentInstruction->getNode(), newMR, prevInstruction);
                break;
            case TR::Instruction::IsMemSrc1:
                prevInstruction = generateMemSrc1Instruction(cg, currentInstruction->getOpCodeValue(),
                    currentInstruction->getNode(), newMR,
                    static_cast<TR::PPCMemSrc1Instruction *>(currentInstruction)->getSourceRegister(), prevInstruction);
                break;
            case TR::Instruction::IsTrg1Mem:
                prevInstruction = generateTrg1MemInstruction(cg, currentInstruction->getOpCodeValue(),
                    currentInstruction->getNode(),
                    static_cast<TR::PPCTrg1MemInstruction *>(currentInstruction)->getTargetRegister(), newMR,
                    prevInstruction);
                break;
            default:
                TR_ASSERT_FATAL_WITH_INSTRUCTION(currentInstruction, false,
                    "Unrecognized instruction kind %d for memory instruction", currentInstruction->getKind());
        }
    }
...
TR::Instruction *OMR::Power::MemoryReference::expandInstruction(TR::Instruction *currentInstruction,
    TR::CodeGenerator *cg)
{
    ...
    if (!self()->getBaseRegister() && !self()->getLabel()) {
        ...
        TR_ASSERT_FATAL_WITH_INSTRUCTION(currentInstruction, !self()->getModBase(),
            "Cannot have mod base and base register");
    }
    ...
    TR_ASSERT_FATAL_WITH_INSTRUCTION(currentInstruction,
    ...
    if (self()->getStaticRelocation() != NULL) {
        prevInstruction
            = generateLabelInstruction(cg, TR::InstOpCode::label, node, generateLabelSymbol(cg), prevInstruction);
    ...
    if (comp->target().is64Bit() && self()->isTOCAccess()) {
        ...
        if (symbol->isStatic())
            ...
            TR_ASSERT_FATAL_WITH_INSTRUCTION(currentInstruction, false,
                "Symbol %s has unrecognized type for PTOC access", comp->getDebug()->getName(symbol));
        ...
        if (displacement != PTOC_FULL_INDEX) {
            ...
            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, data, base, cg->hiValue(displacement),
                    prevInstruction);
            ...
            if (cg->comp()->compileRelocatableCode() && symbol->isStatic() && symbol->isClassObject()) {
                prevInstruction = generateLabelInstruction(cg, TR::InstOpCode::label, node, generateLabelSymbol(cg),
                    prevInstruction);

                TR_RelocationRecordInformation *recordInfo
                    = (TR_RelocationRecordInformation *)cg->trMemory()->allocateHeapMemory(
                ...
                if (comp->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    cg->addExternalRelocation(new (cg->trHeapMemory())
                                                  TR::BeforeBinaryEncodingExternalRelocation(prevInstruction,
                                                      (uint8_t *)recordInfo, TR_DiscontiguousSymbolFromManager, cg),
                    ...
                    cg->addExternalRelocation(new (cg->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                                                  prevInstruction, (uint8_t *)recordInfo, TR_ClassAddress, cg),
                        __FILE__, __LINE__, node);
                }
            ...
            TR::Instruction *newInstruction = prevInstruction = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis,
                node, data, (int16_t)(addr >> 48), prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, data, data,
                (addr >> 32) & 0xffff, prevInstruction);
            prevInstruction = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, data, data, 32,
                0xffffffff00000000ULL, prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::oris, node, data, data,
                (addr >> 16) & 0xffff, prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, data, data, addr & 0xffff,
                prevInstruction);
        ...
        if (index != NULL) {
            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                prevInstruction = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, index,
                    (int16_t)(displacement >> 16), prevInstruction);
                prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, index, index,
                    displacement & 0xffff, prevInstruction);
            } else if (displacement != 0) {
                prevInstruction
                    = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, index, displacement, prevInstruction);
            } else {
                // To implicitly use a displacement of 0, put the base register into the index position and use r0 as
                // the base register.
                self()->setIndexRegister(base);
                self()->setBaseRegister(cg->machine()->getRealRegister(TR::RealRegister::gr0));
            }
        } else {
            TR_ASSERT_FATAL_WITH_INSTRUCTION(currentInstruction, self()->isBaseModifiable(),
                "Delayed index form without index register requires isBaseModifiable");

            if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
                prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, base, base,
                    cg->hiValue(displacement), prevInstruction);
                displacement = LO_VALUE(displacement);
            }

            if (displacement != 0)
                prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, base, base,
                    displacement, prevInstruction);
        ...
        && currentInstruction->getOpCode().getMaxBinaryLength() == 4) {
        if (self()->isBaseModifiable()) {
            prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, base, base,
                cg->hiValue(displacement), prevInstruction);
            self()->setOffset(LO_VALUE(displacement));
            ...
            prevInstruction = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                TR::MemoryReference::createWithDisplacement(cg, stackPtr, -saveLen, saveLen), rX, prevInstruction);
            prevInstruction = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, rX, base,
                cg->hiValue(displacement), prevInstruction);

            self()->setBaseRegister(rX);
            self()->setOffset(LO_VALUE(displacement));

            currentInstruction = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, rX,
                TR::MemoryReference::createWithDisplacement(cg, stackPtr, -saveLen, saveLen), currentInstruction);
        }
    }
...
static TR::RealRegister::RegNum choose_rX(TR::Instruction *currentInstruction, TR::RealRegister *base)
{
    ...
    int32_t exclude = ((TSnum == TR::RealRegister::gr10) ? 1 : 0) | ((TSnum == TR::RealRegister::gr9) ? 2 : 0)
        | ((basenum == TR::RealRegister::gr10) ? 1 : 0) | ((basenum == TR::RealRegister::gr9) ? 2 : 0);
    switch (exclude) {
        case 0:
        case 2:
...
void OMR::Power::MemoryReference::accessStaticItem(TR::Node *node, TR::SymbolReference *ref, bool isStore,
    TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    TR::Symbol *symbol;
    ...
    if (useUnresSnippetToAvoidRelo) {
        ...
        bool reloIsRequired = symbol->isStatic() && symbol->isClassObject() && ref->getCPIndex() == -1;
    ...
    if (cg->comp()->target().is64Bit()) {
        ...
        if (symbol->isCountForRecompile() && cg->needRelocationsForPersistentInfoData()) {
            ...
            TR_ASSERT_FATAL(!comp->getOption(TR_UseSymbolValidationManager) || ref->isUnresolved(),
                "SVM relocation unhandled");
        }
        ...
        if (cg->comp()->target().cpu.isAtLeast(OMR_PROCESSOR_PPC_P10)) {
            ...
            if (ref->isUnresolved() || useUnresSnippetToAvoidRelo
                || (cg->comp()->compileRelocatableCode() && symbol->isStatic() && symbol->isClassObject())) {
                ...
                TR::MemoryReference *fakeTocRef
                    = TR::MemoryReference::createWithDisplacement(cg, NULL, 0, sizeof(uintptr_t));
            ...
            //  (otherwise optimizer will remove the ResolveCHK and we can be sure pTOC will contain the resolved
            //  address),
            ...
            TR::MemoryReference *tocRef
                = TR::MemoryReference::createWithDisplacement(cg, cg->getTOCBaseRegister(), 0, sizeof(uintptr_t));
        ...
        if ((refIsUnresolved || comp->compileRelocatableCode()) && symbol->isDebugCounter()) {
            TR::Register *reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol, 1, reg, NULL, false,
                TR_DebugCounter);
            return;
        } else if ((refIsUnresolved || cg->needRelocationsForPersistentInfoData()) && symbol->isCountForRecompile()) {
            TR::Register *reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(cg, cg->needRelocationsForPersistentInfoData(), nodeForSymbol, TR_CountForRecompile,
                reg, NULL, false, TR_GlobalValue);
            return;
        } else if ((refIsUnresolved || cg->needRelocationsForBodyInfoData()) && symbol->isRecompilationCounter()) {
            TR::Register *reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(cg, cg->needRelocationsForBodyInfoData(), nodeForSymbol, 0, reg, NULL, false,
            ...
            TR::Register *reg = _baseRegister = cg->allocateRegister();
            loadAddressConstant(cg, cg->needRelocationsForCurrentMethodPC(), nodeForSymbol, 0, reg, NULL, false,
            ...
            self()->setUnresolvedSnippet(
                new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(cg, node, ref, isStore, false));
            cg->addSnippet(self()->getUnresolvedSnippet());
            return;
        }
        ...
        if (!node)
            node = cg->getCurrentEvaluationTreeTop()->getNode();
        ...
        if (cg->needClassAndMethodPointerRelocations()) {
            TR_ASSERT_FATAL(symbol->isClassObject(), "expected a class symbol for symref #%d\n",
            ...
            staticRelocation = new (cg->trHeapMemory())
                TR::PPCPairedRelocation(rel1, NULL, (uint8_t *)recordInfo, TR_ClassAddress, node);
...
void OMR::Power::MemoryReference::setOffsetRequiresWordAlignment(TR::Node *node, TR::CodeGenerator *cg,
...
void OMR::Power::MemoryReference::setSymbol(TR::Symbol *symbol, TR::CodeGenerator *cg)
{
...
void OMR::Power::MemoryReference::checkRegisters(TR::CodeGenerator *cg)
{
    if (_baseRegister != NULL && _indexRegister != NULL
        && (self()->hasDelayedOffset() || self()->getOffset(*cg->comp()) != 0)) {
        self()->consolidateRegisters(NULL, NULL, false, cg);
...
int64_t OMR::Power::MemoryReference::getOffset(TR::Compilation &comp)
{
